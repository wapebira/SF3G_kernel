...
/* Required properties */
	...
	if (of_property_read_u32_array(np, OF_CFG_B100,
		platdata->cfg_baud[I2C_XGOLD_B100], 7) < 0) {
	...
	if (of_property_read_u32_array(np, OF_CFG_B400,
		platdata->cfg_baud[I2C_XGOLD_B400], 7) < 0) {
					...
					unsigned int flags)
{
	...
	if (cbaud != data->current_baud) {
		...
		reg |= (pdata->cfg_baud[cbaud][TIM_SDA] <<
			...
			(pdata->cfg_baud[cbaud][EN_SCL_LOW_LEN] <<
				I2C_TIM_CFG_EN_SCL_LOW_LEN_OFFSET) |
			(pdata->cfg_baud[cbaud][FS_SCL_LOW] <<
				I2C_TIM_CFG_FS_SCL_LOW_OFFSET) |
			(pdata->cfg_baud[cbaud][SCL_LOW_LEN] <<
				I2C_TIM_CFG_SCL_LOW_LEN_OFFSET);
...
static void xgold_i2c_show_regs(struct i2c_adapter *adap)
{
	struct xgold_i2c_algo_data *data = i2c_get_adapdata(adap);
	unsigned int reg;

	/*I2C_CLC, I2C Clock Control Register*/
	reg = ioread32(data->regs + I2C_CLC_OFFSET);
	dev_info(&adap->dev, "I2C_CLC: 0x%08x\n", reg);
	/*I2C_ID, I2C Identification Register*/
	reg = ioread32(data->regs + I2C_ID_OFFSET);
	dev_info(&adap->dev, "I2C_ID: 0x%08x\n", reg);

	/*I2C_RUN_CTRL, I2C_ENDD_CTRL, I2C_FDIV_CFG,
	I2C_FDIV_HIGH_CFG, I2C_ADDR_CFG, I2C_BUS_STAT,
	I2C_FIFO_CFG, I2C_MRPS_CTRL,I2C_RPS_STAT,
	I2C_TPS_CTRL, I2C_FFS_STAT Register*/
	reg = ioread32(data->regs + I2C_RUN_CTRL_OFFSET);
	dev_info(&adap->dev, "I2C_RUN_CTRL: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ENDD_CTRL_OFFSET);
	dev_info(&adap->dev, "I2C_ENDD_CTRL: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_FDIV_CFG_OFFSET);
	dev_info(&adap->dev, "I2C_FDIV_CFG: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_FDIV_HIGH_CFG_OFFSET);
	dev_info(&adap->dev, "I2C_FDIV_HIGH_CFG: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ADDR_CFG_OFFSET);
	dev_info(&adap->dev, "I2C_ADDR_CFG: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_BUS_STAT_OFFSET);
	dev_info(&adap->dev, "I2C_BUS_STAT: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_FIFO_CFG_OFFSET);
	dev_info(&adap->dev, "I2C_FIFO_CFG: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_MRPS_CTRL_OFFSET);
	dev_info(&adap->dev, "I2C_MRPS_CTRL: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_RPS_STAT_OFFSET);
	dev_info(&adap->dev, "I2C_RPS_STAT: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_TPS_CTRL_OFFSET);
	dev_info(&adap->dev, "I2C_TPS_CTRL: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_FFS_STAT_OFFSET);
	dev_info(&adap->dev, "I2C_FFS_STAT: 0x%08x\n", reg);

	/*I2C_TIM_CFG, I2C Timing Configuration Register*/
	reg = ioread32(data->regs + I2C_TIM_CFG_OFFSET);
	dev_info(&adap->dev, "I2C_TIM_CFG: 0x%08x\n", reg);

	/*I2C_GP_CTRL, I2C General Purpose Control Register*/
	reg = ioread32(data->regs + 0X44);
	dev_info(&adap->dev, "I2C_GP_CTRL: 0x%08x\n", reg);

	/*I2C_ERR_IRQSM, I2C_ERR_IRQSS, I2C_ERR_IRQSC Register*/
	reg = ioread32(data->regs + I2C_ERR_IRQSM_OFFSET);
	dev_info(&adap->dev, "I2C_ERR_IRQSM: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ERR_IRQSS_OFFSET);
	dev_info(&adap->dev, "I2C_ERR_IRQSS: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ERR_IRQSC_OFFSET);
	dev_info(&adap->dev, "I2C_ERR_IRQSC: 0x%08x\n", reg);

	/*I2C_P_IRQSM, I2C_P_IRQSS, I2C_P_IRQSC Register*/
	reg = ioread32(data->regs + I2C_P_IRQSM_OFFSET);
	dev_info(&adap->dev, "I2C_P_IRQSM: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_P_IRQSS_OFFSET);
	dev_info(&adap->dev, "I2C_P_IRQSS: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_P_IRQSC_OFFSET);
	dev_info(&adap->dev, "I2C_P_IRQSC: 0x%08x\n", reg);

	/*I2C_RIS, I2C_IMSC, I2C_MIS, I2C_ICR, I2C_ISR Register*/
	reg = ioread32(data->regs + I2C_RIS_OFFSET);
	dev_info(&adap->dev, "I2C_RIS: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_IMSC_OFFSET);
	dev_info(&adap->dev, "I2C_IMSC: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_MIS_OFFSET);
	dev_info(&adap->dev, "I2C_MIS: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ICR_OFFSET);
	dev_info(&adap->dev, "I2C_ICR: 0x%08x\n", reg);

	reg = ioread32(data->regs + I2C_ISR_OFFSET);
	dev_info(&adap->dev, "I2C_ISR: 0x%08x\n", reg);
}
...
static irqreturn_t xgold_i2c_err_handler(void *dev)
{
	...
	struct xgold_i2c_algo_data *data = i2c_get_adapdata(adap);
	unsigned int err_irqss = ioread32(data->regs + I2C_ERR_IRQSS_OFFSET);

	if (!(err_irqss))
		return IRQ_NONE;

	dev_err(&adap->dev, "%s, device: 0x%x ERR_INT 0x%x "
		/*more info for debug*/
		"recvd msg len: %d, state: %d\n",
		__func__, data->addr, err_irqss,
		data->msg->len, data->state);
	xgold_i2c_show_regs(adap);
	...
	do {
		...
		if (mis & (I2C_MIS_LSREQ_INT_INT_PEND |
			...
			if (data->state == XGOLD_I2C_TRANSMIT) {
				...
				if (I2C_MSG_RD(data->flags))
					if (0 != mv_svc_enter_atomic_context())
						dev_err(&adap->dev, "mv_svc_enter_atomic_context failed\n");
				...
				if (I2C_MSG_RD(data->flags))
					if (0 != mv_svc_exit_atomic_context())
						dev_err(&adap->dev, "mv_svc_exit_atomic_context failed\n");
			} else {
		...
		struct i2c_msg msgs[], int num)
{
	...
	rt_mutex_lock(&data->i2c_suspend_lock);
	if (data->suspended) {
		dev_err(&adap->dev, "xgold i2c bus is not initialized.\n");
		rt_mutex_unlock(&data->i2c_suspend_lock);
		return -EIO;
	}

	spin_lock_irqsave(&data->lock, flags);
	if (data->state != XGOLD_I2C_LISTEN) {
		dev_err(&adap->dev, "Transaction ongoing, error state: %d, bus id: %d, addr[0x%x]\n",
			data->state, adap->nr, msgs[0].addr);
		spin_unlock_irqrestore(&data->lock, flags);
		rt_mutex_unlock(&data->i2c_suspend_lock);
		return -EAGAIN;
	}
	...
	if (xgold_i2c_bus_status(data) != XGOLD_I2C_BUS_FREE) {
		pr_info_ratelimited("BUSY\n");
		pr_info_ratelimited("bus id: %d,addr[0x%x],Driver state %d, Bus state %u.\n",
			adap->nr, msgs[0].addr, data->state,
...
static void xgold_i2c_hw_init(struct xgold_i2c_algo_data *data)
{
	...
	reg |= (pdata->cfg_baud[data->current_baud][TIM_SDA] <<
		...
		(pdata->cfg_baud[data->current_baud][TIM_HS_SDA] <<
			I2C_TIM_CFG_HS_SDA_DEL_HD_DAT_OFFSET) |
		(pdata->cfg_baud[data->current_baud][EN_SCL_LOW_LEN] <<
			I2C_TIM_CFG_EN_SCL_LOW_LEN_OFFSET) |
		(pdata->cfg_baud[data->current_baud][FS_SCL_LOW] <<
			I2C_TIM_CFG_FS_SCL_LOW_OFFSET) |
		(pdata->cfg_baud[data->current_baud][SCL_LOW_LEN] <<
			I2C_TIM_CFG_SCL_LOW_LEN_OFFSET);
	reg_write(reg, data->regs + I2C_TIM_CFG_OFFSET);
...
static int xgold_i2c_core_probe(struct device *dev)
{
	...
	rt_mutex_init(&algo_data->i2c_suspend_lock);
