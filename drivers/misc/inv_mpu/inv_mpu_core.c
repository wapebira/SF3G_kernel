...
static int inv_write_accel_fs(struct inv_mpu_iio_s *st, int fs)
{
	...
	if (fs == st->chip_config.accl_fs)
		return 0;
	if (INV_MPU3050 == st->chip_type)
//--yd 		result = st->mpu_slave->set_fs(st, fs);
		result = st->mpu_slave->set_fs1(st, fs);
	else
		result = inv_i2c_single_write(st, reg->accl_config,
...
/**
 *  inv_detect_akm_exist()
 */
static int inv_detect_akm_exist(struct inv_mpu_iio_s *st)
{
...
//printk("mpu_parse_dt  11 irq_pin=%d   \n",irq_pin);
	...
	ret = of_property_read_u32(np,"mpu-int_config",(u32 *)&data->int_config);
	if(ret!=0){
		dev_err(&client->dev, "get mpu-int_config error\n");
		return -EIO;
	}

	ret = of_property_read_u32(np,"mpu-level_shifter",(u32 *)&data->level_shifter);
	if(ret!=0){
	...
	ret = of_property_read_u32(np,"sec_slave_type",&(data->sec_slave_type));
	if(ret!=0){
		dev_err(&client->dev, "get sec_slave_type error\n");
		return -EIO;
	}

	ret = of_property_read_u32(np,"sec_slave_addr",&(data->secondary_i2c_addr));
	if(ret!=0){
		dev_err(&client->dev, "get secondary_i2c_addr error\n");
		return -EIO;
	}

	ret = of_property_read_u32(np,"sec_slave_id",&(data->sec_slave_id));
	if(ret!=0){
		dev_err(&client->dev, "get sec_slave_id error\n");
		return -EIO;
	}


	ret = of_property_read_u32(np,"sec_layout",&(data->sec_layout));
	if(ret!=0){
		dev_err(&client->dev, "get sec_slave_id error\n");
		return -EIO;
	}
	...
	switch(data->sec_layout)
	{
		case 1:
			data->secondary_orientation[0] = 1;
			data->secondary_orientation[1] = 0;
			data->secondary_orientation[2] = 0;

			data->secondary_orientation[3] = 0;
			data->secondary_orientation[4] = 1;
			data->secondary_orientation[5] = 0;

			data->secondary_orientation[6] = 0;
			data->secondary_orientation[7] = 0;
			data->secondary_orientation[8] = 1;
			break;

		case 2:
			data->secondary_orientation[0] = 0;
			data->secondary_orientation[1] = -1;
			data->secondary_orientation[2] = 0;

			data->secondary_orientation[3] = 1;
			data->secondary_orientation[4] = 0;
			data->secondary_orientation[5] = 0;
	...
	if(debug){
		...
		printk("data->sec_slave_type=%d,data->sec_slave_id=%d,data->secondary_i2c_addr=%x \n",data->sec_slave_type, \
			data->sec_slave_id,data->secondary_i2c_addr);

		for(i=0;i<size;i++)
			printk("%d ",data->secondary_orientation[i]);	
		printk("\n");	
	...
	const struct i2c_device_id *id)
{
	...
	int result;
	int ret = 0;
	printk("inv_mpu_probe    client->irq=%d\n",client->irq);
...
//--yd 	indio_dev = iio_allocate_device(sizeof(*st));
	...
	ret = mpu_parse_dt(client,&st->plat_data);
	if(ret< 0)
