...
static bool oct_offlog_get_config(char *cfg_path)
{
	...
	snprintf(value_path, OFFLOG_CONFIG_MAX_VALUE_LENGTH,
		"%s%d%02d%02d_%02d%02d%02d/", value_path,
...
bool oct_offlog_check_dir(void)
{
	mm_segment_t old_fs;
	int ret = 0;
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	ret = sys_statfs(oct_offlog_path, NULL);
	set_fs(old_fs);
	if (ret != -ENOENT)
		return true;
	else
