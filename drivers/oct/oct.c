...
static int pm_suspend_fct(struct device *dev)
{
	...
	/* enable OCTM Ring buffer mode */
	SET_OCT_OCT_CNF_ENABLE_OCTM_RINGBF(oct_trform,
		OCT_CNF_ENABLE_OCTM_RINGBF_ENABLED);
	/* trace_debug_oct_dma_off(); */

	SET_OCT_OCT_CNF_ENABLE_OCTM_RINGBF(oct_trform, OCT_CNF_ENABLE_OCTM_RINGBF_ENABLED);
...
static int oct_driver_probe(struct platform_device *pdev)
{
	...
	OCT_LOG("Kernel OCT Thread: %s", task->comm);
