...
static struct xgold_irq_chip_data *irq_eint_data = NULL;
...
struct xgold_irq_chip_data *rockchip_thermal_get_data(void)
{
	BUG_ON(!irq_eint_data);
	return irq_eint_data;
}
...
#ifdef CONFIG_PM
static int xgold_irq_eint_syscore_suspend(void)
{
	struct xgold_irq_chip_data *chipdata = rockchip_thermal_get_data();
	u32 irq = 0;

	chipdata->saved_mask = 0;
	for(irq = 0; irq < chipdata->nr_int; irq++) {
		if (xgold_irq_read(chipdata, chipdata->status[irq])) {
			chipdata->saved_mask |= (0x1 << irq);
		} else {
			if (chipdata->saved_wake & (0x1 << irq)) {
				/*xgold_irq_eint_unmask(d);*/
				if (chipdata->mask[irq]) {
					spin_lock(&eint_lock);
					/*no need to do ack*/
					/*if (chipdata->preack[irq]) {
						xgold_irq_eint_ack(d);
					}*/
					if (chipdata->unmask[irq])
						xgold_irq_write(chipdata,
							chipdata->mask[irq], 1, XGOLD_WO);
...
static void xgold_irq_eint_syscore_resume(void)
{
	struct xgold_irq_chip_data *chipdata = rockchip_thermal_get_data();
	u32 irq = 0;
	...
	for(irq = 0; irq < chipdata->nr_int; irq++) {
		if (xgold_irq_read(chipdata, chipdata->status[irq])) {
			if (!(chipdata->saved_mask & (0x1 << irq))) {
				/*xgold_irq_eint_mask(d);*/
				if (chipdata->mask[irq]) {
					spin_lock(&eint_lock);
					if (chipdata->unmask[irq])
						xgold_irq_write(chipdata,
...
static struct syscore_ops xgold_irq_eint_syscore_ops = {
	.suspend = xgold_irq_eint_syscore_suspend,
	.resume = xgold_irq_eint_syscore_resume,
};
					...
					struct device_node *parent)
{
	...
	if(!ret) {
		irq_eint_data = data;
		register_syscore_ops(&xgold_irq_eint_syscore_ops);
	}
