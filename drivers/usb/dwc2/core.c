...
/**
 * dwc2_restore_registers() - Restore controller registers.
 * When resuming usb bus, device registers needs to be restored
 * if controller power were disabled.
 *
 * @hsotg: Programming view of the DWC_otg controller
 */
static int dwc2_restore_registers(struct dwc2_hsotg *hsotg)
{
...
/**
 * dwc2_backup_registers() - Backup controller registers.
 * When suspending usb bus, device registers needs to be backuped
 * if controller power is disabled once suspended.
 *
 * @hsotg: Programming view of the DWC_otg controller
 */
static int dwc2_backup_registers(struct dwc2_hsotg *hsotg)
{
	struct gregs_backup *gr;
	struct dregs_backup *dr;
	int i;
	...
	gr = hsotg->gr_backup;
	if (!gr) {
		gr = kmalloc(sizeof(*gr), GFP_ATOMIC);
		if (!gr) {
...
int dwc2_exit_hibernation(struct dwc2_hsotg *hsotg, bool restore)
{
	...
	udelay(100);
	if (restore) {
		ret = dwc2_restore_registers(hsotg);
		if (ret) {
...
int dwc2_enter_hibernation(struct dwc2_hsotg *hsotg)
{
	...
	if (ret) {
		dev_err(hsotg->dev, "%s: failed to backup registers\n",
				__func__);
		return ret;
	}
