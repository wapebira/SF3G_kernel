...
struct mtp_dev {
	...
	int xfer_result;
	/*used for pre-allocate memory*/
	void *rx_buf[RX_REQ_MAX];
	void *tx_buf[TX_REQ_MAX];
	void *intr_buf[INTR_REQ_MAX];
};
				...
				struct usb_endpoint_descriptor *intr_desc)
{
	...
	for (i = 0; i < RX_REQ_MAX; i++) {
		...
		req->buf = dev->rx_buf[i];
		req->complete = mtp_complete_out;
		dev->rx_req[i] = req;
	}
	for (i = 0; i < INTR_REQ_MAX; i++) {
		req = usb_ep_alloc_request(dev->ep_intr, GFP_KERNEL);
		if (!req)
			goto fail;
		if (!dev->intr_buf[i]) {
			usb_ep_free_request(dev->ep_intr, req);
			goto fail;
		}
		req->buf = dev->intr_buf[i];
...
static int __mtp_setup(struct mtp_instance *fi_mtp)
{
	...
	for (i = 0; i < TX_REQ_MAX; i++) {
		dev->tx_buf[i] = kmalloc(MTP_BULK_BUFFER_SIZE,
					 GFP_KERNEL);
		if (!dev->tx_buf[i]) {
			ret = -ENOMEM;
			goto err1;
	...
	for (i = 0; i < RX_REQ_MAX; i++) {
		dev->rx_buf[i] = kmalloc(MTP_BULK_BUFFER_SIZE,
					 GFP_KERNEL);
		if (!dev->rx_buf[i]) {
			ret = -ENOMEM;
			goto err1;
	...
	for (i = 0; i < INTR_REQ_MAX; i++) {
		dev->intr_buf[i] = kmalloc(MTP_BULK_BUFFER_SIZE,
					   GFP_KERNEL);
		if (!dev->intr_buf[i]) {
			ret = -ENOMEM;
			goto err1;
