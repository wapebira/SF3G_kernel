...
static int intel_otg_suspend(struct intel_usbphy *iphy)
{
	...
	if (device_bus_suspend || host_bus_suspend) {
		/* restore usb power, disable usb clks and isolation */
...
static void intel_otg_notify_charger(struct intel_usbphy *iphy, unsigned mA)
{
	...
	dev_info(iphy->phy.dev, "Avail curr from USB = %u\n", mA);
...
#define CHG_DCD_TIMEOUT		(900 * HZ/1000) /* 900 msec */
#define CHG_DCD_POLL_TIME	(300 * HZ/1000) /* 300 msec */
#define CHG_PRIMARY_DET_TIME	(80 * HZ/1000) /* TVDPSRC_ON */
#define CHG_SECONDARY_DET_TIME	(80 * HZ/1000) /* TVDMSRC_ON */
...
static void intel_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_WAIT_FOR_DCD:
		...
		if (is_dcd || iphy->tmout) {
			...
			/* Probe on DM, Voltage Source on DP */
