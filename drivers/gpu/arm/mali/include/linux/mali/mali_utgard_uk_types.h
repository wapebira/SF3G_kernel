...
} __aligned(8) _mali_uk_gp_suspend_response_s;
...
typedef struct {
	...
	_mali_uk_fence_t __aligned(8) fence;           /**< [in] fence this job must wait on */
	u64  __aligned(8) timeline_point_ptr;            /**< [in,out] pointer to u32: location where point on gp timeline for this job will be written */
} __aligned(8) _mali_uk_gp_start_job_s;
...
typedef struct {
	...
	u64 __aligned(8) memory_cookies;            /**< [in] pointer to array of u32 memory cookies attached to job */
	_mali_uk_fence_t fence;             /**< [in] fence this job must wait on */
	u64  __aligned(8) timeline_point_ptr;           /**< [in,out] pointer to location of u32 where point on pp timeline for this job will be written */
} __aligned(8) _mali_uk_pp_start_job_s;
...
} __aligned(8) _mali_uk_pp_and_gp_start_job_s;
...
typedef struct {
	...
	u32 wb1_memory;
	u32 wb2_memory;
} __aligned(8) _mali_uk_pp_disable_wb_s;
...
} __aligned(8) _mali_uk_soft_job_start_s;
...
} __aligned(8) _mali_uk_soft_job_signal_s;
...
} __aligned(8) _mali_uk_wait_for_notification_s;
...
} __aligned(8) _mali_uk_post_notification_s;
...
typedef struct {
	...
	_mali_uk_api_version version;   /**< [in,out] API version of user-side interface. */
	int compatible;                 /**< [out] @c 1 when @version is compatible, @c 0 otherwise */
} __aligned(8) _mali_uk_get_api_version_v2_s;
...
} __aligned(8) _mali_uk_get_user_settings_s;
...
typedef struct {
	u64 ctx;                       /**< [in,out] user-kernel context (trashed on output) */
} __aligned(8) _mali_uk_request_high_priority_s;
...
} __aligned(8) _mali_uk_map_external_mem_s;

typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	u32 cookie;                     /**< [out] identifier for mapped memory object in kernel space  */
} __aligned(8) _mali_uk_unmap_external_mem_s;
...
typedef struct {
	...
	u32 flags;                      /**< [in] flags, see \ref _MALI_MAP_EXTERNAL_MAP_GUARD_PAGE */
	u32 cookie;                     /**< [out] identifier for mapped memory object in kernel space  */
} __aligned(8) _mali_uk_attach_dma_buf_s;

typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	u32 mem_fd;                     /**< [in] Memory descriptor */
	u32 size;                       /**< [out] size */
} __aligned(8) _mali_uk_dma_buf_get_size_s;

typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	u64 cookie;                     /**< [in] identifier for mapped memory object in kernel space  */
} __aligned(8) _mali_uk_release_dma_buf_s;
...
typedef struct {
	...
	u32 flags;                      /**< [in] flags, see \ref _MALI_MAP_EXTERNAL_MAP_GUARD_PAGE */
	u32 cookie;                     /**< [out] identifier for mapped memory object in kernel space  */
} __aligned(8) _mali_uk_attach_ump_mem_s;

typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	u32 cookie;                     /**< [in] identifier for mapped memory object in kernel space  */
} __aligned(8) _mali_uk_release_ump_mem_s;
...
} __aligned(8) _mali_uk_mem_write_safe_s;

typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	u32 size;                       /**< [out] size of MMU page table information (registers + page tables) */
} __aligned(8) _mali_uk_query_mmu_page_table_dump_size_s;
...
} __aligned(8) _mali_uk_dump_mmu_page_table_s;
...
} __aligned(8) _mali_uk_get_pp_number_of_cores_s;
...
typedef struct {
	...
	u32 padding;
} __aligned(8) _mali_uk_get_pp_core_version_s;
...
} __aligned(8) _mali_uk_get_gp_number_of_cores_s;
...
typedef struct {
	u64 ctx;                      /**< [in,out] user-kernel context (trashed on output) */
	_mali_core_version version;     /**< [out] version returned from core, see \ref _mali_core_version */
} __aligned(8) _mali_uk_get_gp_core_version_s;
...
} __aligned(8) _mali_uk_profiling_add_event_s;

typedef struct {
	u64 ctx;                     /**< [in,out] user-kernel context (trashed on output) */
	u32 memory_usage;              /**< [out] total memory usage */
} __aligned(8) _mali_uk_profiling_memory_usage_get_s;
...
} __aligned(8) _mali_uk_vsync_event_report_s;
...
} __aligned(8) _mali_uk_sw_counters_report_s;
...
} __aligned(8) _mali_uk_timeline_get_latest_point_s;
...
} __aligned(8) _mali_uk_timeline_wait_s;
...
} __aligned(8) _mali_uk_timeline_create_sync_fence_s;
