				     ...
				     struct rockchip_fb_win_cfg_data *win_data)
{
	...
	if (!(dev_drv->suspend_flag == 0) || dev_drv->standby_flag) {
...
static int rockchip_fb_close(struct fb_info *info, int user)
{
	...
	if (win_id >= 0) {
		...
		info->fix.line_length =
			ALIGN_N_TIMES(info->var.xres, 32) *
			  ...
			  bool enable, int vop_id)
{
	...
	if (sfb_info->disp_mode == ONE_DUAL || sfb_info->disp_mode == NO_DUAL) {
		if (dev_drv->ops->dsp_black)
			dev_drv->ops->dsp_black(dev_drv, DSP_BLACK_NORMAL);
	}
	...
	if (enable) {
		...
		dev_drv->screen0->ext_screen = dev_drv->cur_screen;
		/* get scaler parameter */
		if (dev_drv->screen0->sscreen_get)
			dev_drv->screen0->sscreen_get(
				dev_drv->screen0,
	...
	/* one vop implement dual display, for example connect rk61x chip */
	if (sfb_info->disp_mode == ONE_DUAL) {
		if (dev_drv->screen0->sscreen_set)
			dev_drv->screen0->sscreen_set(
				dev_drv->screen0, enable);

		if (dev_drv->ops->dsp_black)
			dev_drv->ops->dsp_black(dev_drv, DSP_BLACK_UNBLACK);
	} else if (sfb_info->disp_mode == NO_DUAL) {
		if (!enable) { /* enable vop and transmitter if hdmi plug out */
			if (dev_drv->ops->dsp_black)
				dev_drv->ops->dsp_black(dev_drv,
							DSP_BLACK_UNBLACK);
		} else { /* only enable vop if hdmi plug in */
			if (dev_drv->ops->dsp_black)
				dev_drv->ops->dsp_black(dev_drv,
							DSP_BLACK_TRSM);
		}
		     ...
		     struct rockchip_vop_win *vop_win, int id)
{
	...
	for (i = 0; i < dev_drv->num_win; i++) {
		...
		fbi->fix.line_length = ALIGN_N_TIMES(fbi->var.xres, 32) *
		...
		if (i == 0) {
			...
			set_cpus_allowed(dev_drv->vsync_info.thread, cpumask_of_cpu(0));
			...
			dev_drv->update_regs_thread =
			    kthread_run(kthread_worker_fn,
					...
					"rockchip-fb");
			set_cpus_allowed(dev_drv->update_regs_thread, cpumask_of_cpu(0));
