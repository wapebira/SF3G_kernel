				    ...
				    bool initscreen)
{
	...
	case SCREEN_RGB:
		...
		break;
	case SCREEN_LVDS:
		if (dev_drv->trsm_ops && dev_drv->trsm_ops->get_property &&
		    dev_drv->trsm_ops->get_property(0) == PROP_EXTER_CONNECT) {
			mask |= M_RGB_DCLK_INVERT;
			val = V_RGB_DCLK_EN(1) | V_RGB_DCLK_INVERT(0);
		} else {
...
static int
rockchip_vop_dsp_black(struct rockchip_vop_driver *dev_drv, int mode)
{
	switch (mode) {
	case DSP_BLACK_NORMAL:
		if (dev_drv->standby_flag)
			return 0;

		dev_drv->standby_flag = 1;
		flush_kthread_worker(&dev_drv->update_regs_worker);

		/* close the backlight */
		if (dev_drv->screen0->backlight_ctrl)
			dev_drv->screen0->backlight_ctrl(dev_drv->screen0,
							 false);

		/* disable DSI/LVDS output */
		if (dev_drv->trsm_ops && dev_drv->trsm_ops->disable)
			dev_drv->trsm_ops->disable();

		rockchip_vop_standby(dev_drv, true);
		break;
	case DSP_BLACK_UNBLACK:
		if (!dev_drv->standby_flag)
			return 0;

		/* enable DSI/LVDS output */
		if (dev_drv->trsm_ops && dev_drv->trsm_ops->enable)
			dev_drv->trsm_ops->enable();

		dev_drv->standby_flag = 0;
		rockchip_vop_standby(dev_drv, false);
		msleep(20);
...
static struct rockchip_vop_drv_ops vop_drv_ops = {
	...
	.dsp_black = rockchip_vop_dsp_black,
};
