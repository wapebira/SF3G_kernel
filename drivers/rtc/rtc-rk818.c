...
static int rk818_rtc_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
...
static int rk818_rtc_update_irq_enable(struct device *dev, unsigned int enabled)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
...
static int rk818_rtc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		/*2012-01-01 12:00:00*/
		rk818_rtc_set_time(&pdev->dev, &rk818_tm_def);
	...
	per_irq = rk818->irq_base + RK818_IRQ_RTC_PERIOD;
	alm_irq = rk818->irq_base + RK818_IRQ_RTC_ALARM;

	dev_info(&pdev->dev, "per_irq = %d, alm_irq = %d\n", per_irq, alm_irq);

	/*request rtc and alarm irq of rk818 */
	ret =
	    devm_request_threaded_irq(rk818->dev, per_irq, NULL, rk818_per_irq,
				      IRQF_TRIGGER_RISING | IRQF_ONESHOT,
				      "RTC period", rk818_rtc);
	if (ret != 0) {
	...
	ret =
	    devm_request_threaded_irq(rk818->dev, alm_irq, NULL, rk818_alm_irq,
				      IRQF_TRIGGER_RISING | IRQF_ONESHOT,
				      "RTC alarm", rk818_rtc);
	if (ret != 0) {
	...
	/*
	 * rk818_set_bits(rk818_rtc->rk818, RK818_RTC_STATUS_REG,
			(0x1<< 6),(0x1 <<6));
	 * rk818_set_bits(rk818_rtc->rk818, RK818_RTC_INT_REG,0x0c,0x0c);
	 * rk818_set_bits(rk818_rtc->rk818, RK818_INT_STS_REG1,
			(0x3 << 5),(0x3 <<5));
	 * rk818_set_bits(rk818_rtc->rk818, RK818_INT_STS_MSK_REG1,(0x3 <<5),0);
	 */

	/*so rk818 alarm irq can wake up system*/
				    ...
				    loff_t *offset)
{
	...
	ret = sscanf((const char *)nr_buf, "%d", &nr);
	if (nr > 5 || nr < 0) {
		dev_err(&pdev->dev, "%s:data is error\n", __func__);
		return -EFAULT;
	}
	...
	if (nr == 0) {
		...
		/*2011-01-01 12:00:00*/
		ret = rk818_rtc_set_time(&pdev->dev, &tm);
		if (ret) {
	...
	if (nr == 2) {
		...
		dev_info(&pdev->dev,
			 "Set alarm %4d-%02d-%02d(%d) %02d:%02d:%02d\n",
			 1900 + alrm.time.tm_year, alrm.time.tm_mon + 1,
	...
	if (nr == 3) {
		...
		dev_info(&pdev->dev, "%s:ret=0x%x\n", __func__, ret & 0xff);

		ret = rk818_reg_write(rk818_rtc->rk818, RK818_RTC_STATUS_REG,
				      ret & 0xff);
		if (ret < 0) {
			dev_err(&pdev->dev,
				"%s:Failed to read RTC status: %d\n",
