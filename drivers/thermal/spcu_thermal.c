...
extern int thermal_limit_max_freq(unsigned int frequency, int cpu);

#define DEFAULT_TEMP 40000		/* 40 degrees */
#define MEAS_INTERVAL 0			/* 80 ms */
#define MEAS_DELAY 15			/* us */
...
static int sw_meas_oneshot(struct threshold *threshold, u32 meas_val)
{
	...
	if (!retry || ret) {
		dev_warn(&threshold->tdev->pdev->dev,
			"retry %d, ret %d\n", retry, ret);
		return -1;
	}
...
static void notify_thermal_event(struct spcu_thermal_device *dev, int trip_id)
{
	...
	pr_debug("%s: type=%s, trip_id=%d, temp=%d\n",
...
static void thermal_limit_cpufreq(struct spcu_thermal_device *dev)
{
	int i = 0;
	int j = 0;
	int ret = 0;

	if (dev->temp_freq_table == NULL)
		return;

	for (i = 0; dev->temp_freq_table[i].frequency !=
		CPUFREQ_TABLE_END; i++) {
		if (dev->cached_temp >= dev->temp_freq_table[i].min_temp &&
		    dev->cached_temp < dev->temp_freq_table[i].max_temp) {
			j = i;
			break;
		}
	}
	if (dev->temp_freq_table[i].frequency != CPUFREQ_TABLE_END) {
		ret = thermal_limit_max_freq(dev->temp_freq_table[j].frequency,
					     dev->id);
		if (ret == -1)
			return;
		for (i = 0; i < TRIP_COUNT; i++) {
			if (i == 0) {
				dev->trip[i].temp =
					dev->temp_freq_table[j].min_temp;
				dev->trip[i].state = TRIP_STATE_LOW;
			} else if (i == 1) {
				dev->trip[i].temp =
					dev->temp_freq_table[j].max_temp;
				dev->trip[i].state = TRIP_STATE_HIGH;
			}
		}
	}
}
...
static void check_trips(struct spcu_thermal_device *dev)
{
	...
	if (notify_trip_id >= 0) {
		if (dev->temp_limit_enable == 1)
			thermal_limit_cpufreq(dev);
		else
			   ...
			   struct device_attribute *attr, char *buf)
{
	...
	desc += sprintf(buf + desc, "%d\n", info->temp_limit_enable);
	return desc;
}

static ssize_t store_kernel(
						...
						const char *buf, size_t count)
{
	...
	ret = kstrtouint(buf, 0, &info->temp_limit_enable);
	if (ret < 0) {
		mutex_unlock(&info->lock);
		return -EINVAL;
	}
	if (info->temp_limit_enable == 1)
		thermal_limit_cpufreq(info);
	mutex_unlock(&info->lock);
	return count;
}

static DEVICE_ATTR(kernel, 0644, show_kernel, store_kernel);
				 ...
				 struct spcu_thermal_platform_data *pdata)
{
	...
	dev->temp_limit_enable = pdata->temp_limit_enable;
	dev->temp_freq_table = pdata->temp_freq_table;
	...
	if (dev->temp_limit_enable == 1 && dev->temp_freq_table != NULL) {
		for (i = 0; i < TRIP_COUNT; i++) {
			if (i == 0) {
				dev->trip[i].temp =
					dev->temp_freq_table[0].min_temp;
				dev->trip[i].state = TRIP_STATE_LOW;
			} else if (i == 1) {
				dev->trip[i].temp =
					dev->temp_freq_table[0].max_temp;
				dev->trip[i].state = TRIP_STATE_HIGH;
			}
		}
	} else {
...
static struct temp_freq_table *of_get_temp_freq_table(
struct device_node *dev_node, const char *propname)
{
	struct temp_freq_table *temp_freq_table = NULL;
	...
	temp_freq_table = kzalloc(sizeof(*temp_freq_table) *
			     (nr/3 + 1), GFP_KERNEL);

	val = prop->value;

	for (i = 0; i < nr/3; i++) {
		temp_freq_table[i].min_temp = be32_to_cpup(val++) * 1000;
		temp_freq_table[i].max_temp = be32_to_cpup(val++) * 1000;
		temp_freq_table[i].frequency = be32_to_cpup(val++);
	}

	temp_freq_table[i].min_temp = 0;
	temp_freq_table[i].max_temp = 0;
	temp_freq_table[i].frequency = CPUFREQ_TABLE_END;
				 ...
				 struct spcu_thermal_platform_data *pdata)
{
	...
	of_property_read_u32_index(node, "temp-limit-enable", 0,
				   &pdata->temp_limit_enable);
	pdata->temp_freq_table =
		of_get_temp_freq_table(node, "temp-freq-table");
