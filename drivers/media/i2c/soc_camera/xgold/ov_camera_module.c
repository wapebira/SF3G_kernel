	...
	if (on) {
		...
		if (OV_CAMERA_MODULE_HW_STANDBY == cam_mod->state) {
			...
			if (!IS_ERR_VALUE(ret)) {
				...
				cam_mod->state = OV_CAMERA_MODULE_SW_STANDBY;
				#if 0  /* ddl@rock-chips.com: move check id  to ov_camera_module_init */
...
static int flash_light_ctrl(
		...
		int value)
{
	struct v4l2_subdev *fl_ctrl = NULL;
	struct v4l2_control single_ctrl;
	int torch_on = 0;
	int ret = 0;

	char *flash_driver = pltfrm_camera_module_get_flash_driver_name(sd);

	if (!strcmp(flash_driver, "LM3642")) {
		if (OV_CAMERA_MODULE_POWER_OFF == cam_mod->state) {
		...
		} /*xuhf add in 20150518*/
		fl_ctrl = pltfrm_camera_module_get_fl_ctrl(sd);
		if (IS_ERR_OR_NULL(fl_ctrl)) {
			pltfrm_camera_module_pr_debug(&cam_mod->sd,
				"failed to get LM3642 device,use hardware pin\n");
			return -EINVAL;
		} else {
			single_ctrl.id =
				V4L2_CID_FLASH_LED_MODE;
			single_ctrl.value = value;
			ret = v4l2_subdev_call(fl_ctrl,
				core, s_ctrl, &single_ctrl);
			/*FIX:not sure why should be trigger by gpio*/
			/*return ret;*/
		}
		if (torch_on) {
		    ret = pltfrm_camera_module_set_pm_state(sd, PM_STATE_D3);
			if (!IS_ERR_VALUE(ret)) {
				mdelay(cam_mod->custom.power_up_delays_ms[0]);
				torch_on = 0;
			}
	...
	if (value == V4L2_FLASH_LED_MODE_NONE) {
