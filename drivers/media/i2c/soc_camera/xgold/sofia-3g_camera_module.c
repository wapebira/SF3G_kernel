...
struct pltfrm_camera_module_reuse {
	/* reset */
	bool reset_init_status;
	struct pltfrm_camera_module_gpio *reset_gpio[2];
};
...
struct pltfrm_camera_module_data {
	...
	/* ddl@rock-chips.com: camera module name and len name*/
	struct pltfrm_camera_module_info_s info;
};

/* xuhf@rock-chips.com: camera module reuse gpio*/
struct pltfrm_camera_module_reuse reuse_ctrl;
	...
	struct v4l2_subdev *sd)
{
	...
	for (i = 0; i < ARRAY_SIZE(pdata->gpios); i++) {
		if (gpio_is_valid(pdata->gpios[i].pltfrm_gpio)) {
			if (pdata->gpios[i].label == PLTFRM_CAMERA_MODULE_PIN_RESET) {
                if (!IS_ERR_OR_NULL(reuse_ctrl.reset_gpio[0]) &&
                    !IS_ERR_OR_NULL(reuse_ctrl.reset_gpio[1])) {
    			    if (reuse_ctrl.reset_init_status &&
    				reuse_ctrl.reset_gpio[0]->pltfrm_gpio ==
    					reuse_ctrl.reset_gpio[1]->pltfrm_gpio) {
    					pltfrm_camera_module_pr_err(
    						sd,
    						"reset gpio has been inited, return!\n");
    					continue;
    					}
				}
			} /*add by xuhf 20150523*/
	...
	reuse_ctrl.reset_init_status = 1;
	return 0;
err:
	...
	struct v4l2_subdev *sd)
{
	...
	ret = of_property_read_string(np, "intel,camera-module-facing", &facing);
	if (ret) {
		pltfrm_camera_module_pr_debug(sd,
	...
	if (ret) {
		...
		fl_ctrl_node = of_parse_phandle(np, "intel,fl-ctrl", 0);
	...
	/*xuhf@rock-chips.com: set reset reuse information*/
	if (!strcmp(facing, "back"))
		reuse_ctrl.reset_gpio[0] = &(pdata->gpios[4]);
	else if (!strcmp(facing, "front"))
		reuse_ctrl.reset_gpio[1] = &(pdata->gpios[4]);

	/*ddl@rock-chips.com: set module information*/
	of_property_read_string(np, OF_CAMERA_MODULE_NAME,
			&pdata->info.module_name);
	of_property_read_string(np, OF_CAMERA_MODULE_LEN_NAME,
			&pdata->info.len_name);
...
static int pltfrm_superpix_camera_module_write_reglist_node(
	struct v4l2_subdev *sd,
	struct device_node *config_node)
{
	struct property *reg_table_prop;
	...
	if (!IS_ERR_OR_NULL(reg_table_prop)) {
		...
		ret = pltfrm_superpix_camera_module_write_reglist(
			sd, reg_table, reg_table_num_entries);
		if (IS_ERR_VALUE(ret))
			goto err;
		kfree(reg_table);
...
int pltfrm_superpix_camera_module_read_reg(
		struct v4l2_subdev *sd,
	u16 data_length,
	u8 reg,
	u8 *val)
...
int pltfrm_superpix_camera_module_write_reg(
	struct v4l2_subdev *sd,
	u8 reg, u8 val)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
int pltfrm_superpix_camera_module_write_reglist(
	struct v4l2_subdev *sd,
	const struct pltfrm_camera_module_reg reglist[],
	int len)
{
...
int pltfrm_superpix_camera_module_patch_config(
	struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *frm_fmt,
	struct v4l2_subdev_frame_interval *frm_intrvl)
{
	...
	while (!IS_ERR_OR_NULL(child_node =
		...
		if (0 == strncasecmp(child_node->name,
			...
			if (ret) {
				ret =
					pltfrm_superpix_camera_module_write_reglist_node(
						sd, child_node);
				if (!IS_ERR_VALUE(ret))
					goto err;
			}
