...
static int gc0310_power_down(struct v4l2_subdev *sd)
{
...
static int gc0310_power_up(struct v4l2_subdev *sd)
{
	int ret;
	...
	msleep(20);
	pltfrm_camera_module_pr_debug(sd, "gc0310 power-up done.\n");
	return ret;

fail_power:
	gc0310_power_down(sd);
	pltfrm_camera_module_pr_err(sd, "gc0310 power-up failed\n");
	...
	if (on == 0) {
		...
		if (0 == dev->power) {
            ...
            if (on && !ret) {
                ...
                if (ret) {
                    pltfrm_camera_module_pr_err(sd, "gc_detect err.\n");
                    __gc_s_power(sd, 0);
                } else if (dev->product_info->sensor_id != sensor_id) {
                    pltfrm_camera_module_pr_err(sd, "sensor id didn't match expected\n");
                    __gc_s_power(sd, 0);
                    ret = -EINVAL;
                }
