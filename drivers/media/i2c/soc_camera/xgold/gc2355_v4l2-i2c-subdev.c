...
#define GC2355_TIMING_FRAME_LENGTH_LINES_HIGH_REG   0x95
#define GC2355_TIMING_FRAME_LENGTH_LINES_LOW_REG    0x96    /*window height*/
#define GC2355_TIMING_LINE_LENGTH_PCKL_HIGH_REG     0x97
#define GC2355_TIMING_LINE_LENGTH_PCKL_LOW_REG      0x98    /*window width*/
#define GC2355_COARSE_INTEGRATION_TIME_MIN          1
#define GC2355_COARSE_INTEGRATION_TIME_MAX_MARGIN   6
...
/* Base sensor configs */
/* ======================================================================== */
static const struct gc_camera_module_reg gc2355_init_tab_2MP_20fps[] = {
	/*SYS*/
	...
	/*Mclk:26Mhz@30fps; 403 */
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x06, 0x25},
	...
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x0e, 0xd0},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x0f, 0x06},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x10, 0x58},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x17, 0x17},
	...
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x12, 0xe4},
	...
	/*Crop*/
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x92, 0x03},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x94, 0x05},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x95, 0x04},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x96, 0xc0},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x97, 0x06},
	{GC_CAMERA_MODULE_REG_TYPE_DATA, 0x98, 0x50},
...
static struct gc_camera_module_config gc2355_configs[] = {
	{
		...
		.frm_fmt = {
			.width = 1600,
			.height = 1200,
			.code = V4L2_MBUS_FMT_SRGGB10_1X10/*RAW10*/
		},
		.frm_intrvl = {
			.interval = {
				.numerator = 1,
		...
		.reg_table = (void *)gc2355_init_tab_2MP_20fps,
		.reg_table_num_entries =
			sizeof(gc2355_init_tab_2MP_20fps)
			/
			sizeof(gc2355_init_tab_2MP_20fps[0]),
		.v_blanking_time_us = 60000 /*empirically measured time*/
	},
};
	...
	if ((cam_mod->state == GC_CAMERA_MODULE_SW_STANDBY) ||
		...
		if ((ANALOG_GAIN_1 <= a_gain) && (a_gain < ANALOG_GAIN_2)) {
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_ANALOG_GAIN_REG, 0x00);
			temp = a_gain;
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_PREGAIN_HIGHBITS_REG, temp>>6);
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_PREGAIN_LOWBITS_REG, (temp<<2)&0xfc);
		} else if ((ANALOG_GAIN_2 <= a_gain) &&
			(a_gain < ANALOG_GAIN_3)) {
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_ANALOG_GAIN_REG, 0x01);
			temp = 64*a_gain/ANALOG_GAIN_2;
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_PREGAIN_HIGHBITS_REG, temp>>6);
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_PREGAIN_LOWBITS_REG, (temp<<2)&0xfc);
		} else if ((ANALOG_GAIN_3 <= a_gain) &&
				(a_gain < ANALOG_GAIN_4)) {
			ret |= gc_camera_module_write_reg(cam_mod,
				GC2355_AEC_ANALOG_GAIN_REG, 0x02);
			...
			ret |= gc_camera_module_write_reg(cam_mod,
			    GC2355_AEC_ANALOG_GAIN_REG, 0x03);
		...
		ret |= gc_camera_module_write_reg(cam_mod,
		    GC2355_AEC_EXPO_COARSE_2ND_REG,
		    GC2355_FETCH_2ND_BYTE_EXP(exp_time));
		ret |= gc_camera_module_write_reg(cam_mod,
		    GC2355_AEC_EXPO_COARSE_1ST_REG,
		    GC2355_FETCH_1ST_BYTE_EXP(exp_time));
	}
...
static int gc2355_g_timings(struct gc_camera_module *cam_mod,
			    struct gc_camera_module_timings *timings)
{
	...
	GC2355 Pixel Clock= InMclk * (0xf8 + 1) /2---*/
	timings->vt_pix_clk_freq_hz =
		GC2355_EXT_CLK * (6 + 1) / 2;	/*Pixel Clock*/

	timings->coarse_integration_time_min =
		GC2355_COARSE_INTEGRATION_TIME_MIN;
	timings->coarse_integration_time_max_margin =
		GC2355_COARSE_INTEGRATION_TIME_MAX_MARGIN;

	timings->fine_integration_time_min =
		GC2355_FINE_INTEGRATION_TIME_MIN;
	timings->fine_integration_time_max_margin =
		GC2355_FINE_INTEGRATION_TIME_MAX_MARGIN;
...
static int gc2355_s_ext_ctrls(struct gc_camera_module *cam_mod,
...
static int gc2355_stop_streaming(struct gc_camera_module *cam_mod)
{
	...
	if (IS_ERR_VALUE(ret))
		goto err;
#else
	ret = gc_camera_module_write_reg(cam_mod, 0x10, 0x80);
	if (IS_ERR_VALUE(ret))
		goto err;
#endif
