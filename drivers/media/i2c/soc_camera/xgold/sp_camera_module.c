...
static struct sp_camera_module *to_sp_camera_module(struct v4l2_subdev *sd)
{
	return container_of(sd, struct sp_camera_module, sd);
}

/* ======================================================================== */

static void sp_camera_module_reset(
	struct sp_camera_module *cam_mod)
{
	...
	cam_mod->rotation = 0;
	cam_mod->ctrl_updt = 0;
	cam_mod->state = SP_CAMERA_MODULE_POWER_OFF;
	cam_mod->state_before_suspend = SP_CAMERA_MODULE_POWER_OFF;
}

/* ======================================================================== */

static void sp_camera_module_set_active_config(
	struct sp_camera_module *cam_mod,
	struct sp_camera_module_config *new_config)
{
	...
	if (IS_ERR_OR_NULL(new_config)) {
		...
		cam_mod->ctrl_updt &= SP_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP |
			SP_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN |
			SP_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		if (new_config->auto_exp_enabled !=
			cam_mod->exp_config.auto_exp) {
			cam_mod->ctrl_updt |=
				SP_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			cam_mod->exp_config.auto_exp =
...
static struct sp_camera_module_config *sp_camera_module_find_config(
	struct sp_camera_module *cam_mod,
...
static int sp_camera_module_write_config(
	struct sp_camera_module *cam_mod)
{
	int ret = 0;
...
#if 1//cjl
	ret = pltfrm_superpix_camera_module_write_reglist(&cam_mod->sd,
		cam_mod->active_config->reg_table,
		cam_mod->active_config->reg_table_num_entries);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = pltfrm_superpix_camera_module_patch_config(&cam_mod->sd,
		&cam_mod->frm_fmt,
		&cam_mod->frm_intrvl);
...
int sp_camera_module_try_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct sp_camera_module *cam_mod = to_sp_camera_module(sd);

	pltfrm_camera_module_pr_debug(&cam_mod->sd, "%dx%d, fmt code 0x%04x\n",
		fmt->width, fmt->height, fmt->code);

	if (IS_ERR_OR_NULL(sp_camera_module_find_config(cam_mod, fmt, NULL))) {
		pltfrm_camera_module_pr_debug(&cam_mod->sd,
...
int sp_camera_module_s_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct sp_camera_module *cam_mod =  to_sp_camera_module(sd);
	int ret = 0;
	...
	if (cam_mod->frm_intrvl_valid) {
		sp_camera_module_set_active_config(cam_mod,
			sp_camera_module_find_config(cam_mod,
...
int sp_camera_module_g_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct sp_camera_module *cam_mod =  to_sp_camera_module(sd);
	...
	struct v4l2_subdev_frame_interval *interval)
{
	...
	if (IS_ERR_OR_NULL(sp_camera_module_find_config(cam_mod,
			NULL, &norm_interval))) {
		pltfrm_camera_module_pr_err(&cam_mod->sd,
...
int sp_camera_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	struct sp_camera_module *cam_mod =  to_sp_camera_module(sd);
	...
	if (enable) {
		if (cam_mod->state == SP_CAMERA_MODULE_STREAMING)
			return 0;
		if (IS_ERR_OR_NULL(cam_mod->active_config)) {
			pltfrm_camera_module_pr_err(&cam_mod->sd,
		...
		if (cam_mod->update_config)
			ret = sp_camera_module_write_config(cam_mod);
			if (IS_ERR_VALUE(ret))
				goto err;
		...
		ret = sp_camera_module_ioctl(sd,
					INTEL_VIDIOC_SENSOR_MODE_DATA,
					&timings);

		cam_mod->state = SP_CAMERA_MODULE_SW_STANDBY;
		...
		pltfrm_camera_module_pr_info(&cam_mod->sd, "sp_camera_module_s_stream=%d,cam_mod->update_config=%d,wait_ms=%d\n", enable,cam_mod->update_config,wait_ms);
	}
...
int sp_camera_module_s_power(struct v4l2_subdev *sd, int on)
{
	int ret = 0;
	struct sp_camera_module *cam_mod =  to_sp_camera_module(sd);
	...
	pltfrm_camera_module_pr_info(&cam_mod->sd, "sp_camera_module_s_power=%d\n", on);

	if (on) {
		if (SP_CAMERA_MODULE_POWER_OFF == cam_mod->state) {
			ret = pltfrm_camera_module_s_power(&cam_mod->sd, 1);
			if (!IS_ERR_VALUE(ret)) {
				mdelay(cam_mod->custom.power_up_delays_ms[0]);
				cam_mod->state = SP_CAMERA_MODULE_HW_STANDBY;
			}
		}
		if (SP_CAMERA_MODULE_HW_STANDBY == cam_mod->state) {
		...
		if (SP_CAMERA_MODULE_STREAMING == cam_mod->state) {
			ret = sp_camera_module_s_stream(sd, 0);
			if (!IS_ERR_VALUE(ret))
				cam_mod->state = SP_CAMERA_MODULE_SW_STANDBY;
		}
		if (SP_CAMERA_MODULE_SW_STANDBY == cam_mod->state) {
		...
		if (SP_CAMERA_MODULE_HW_STANDBY == cam_mod->state) {
			...
			if (!IS_ERR_VALUE(ret)) {
				cam_mod->state = SP_CAMERA_MODULE_POWER_OFF;
				sp_camera_module_reset(cam_mod);
			}
		}
	}
...
int sp_camera_module_g_ctrl(struct v4l2_subdev *sd,
...
//	pltfrm_camera_module_pr_info(&cam_mod->sd, " sp_camera_module_g_ctrl id 0x%x\n", ctrl->id);
...
//		pltfrm_camera_module_pr_info(&cam_mod->sd, "sp_camera_module_s_ext_ctrls ctrl->id = %x\n",ctrl->id);

		switch (ctrl->id) {
		case V4L2_CID_GAIN:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_GAIN;
		...
		case V4L2_CID_FLASH_LED_MODE:
			...
			break;
		case V4L2_CID_EXPOSURE:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_EXP_TIME;
			...
			break;
		case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_WB_TEMPERATURE;
			...
			break;
		case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_PRESET_WB;
			...
			break;
		case V4L2_CID_AUTOGAIN:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN;
			...
			break;
		case V4L2_CID_EXPOSURE_AUTO:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			...
			break;
		case V4L2_CID_AUTO_WHITE_BALANCE:
			ctrl_updt = SP_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		...
		case V4L2_CID_FOCUS_ABSOLUTE:
			...
			ctrl_updt =
				SP_CAMERA_MODULE_CTRL_UPDT_FOCUS_ABSOLUTE;
	...
	if (ctrl_cnt &&
		(cam_mod->state == SP_CAMERA_MODULE_STREAMING ||
		cam_mod->state == SP_CAMERA_MODULE_SW_STANDBY)) {
		struct sp_camera_module_ext_ctrls sp_ctrls;

		sp_ctrls.ctrls =
		(struct sp_camera_module_ext_ctrl *)
		kmalloc(ctrl_cnt*sizeof(struct sp_camera_module_ext_ctrl),
			GFP_KERNEL);

		if (sp_ctrls.ctrls) {
			for (i = 0; i < ctrl_cnt; i++) {
				sp_ctrls.ctrls[i].id = ctrls->controls[i].id;
				sp_ctrls.ctrls[i].value =
					ctrls->controls[i].value;
			}

			sp_ctrls.count = ctrl_cnt;

			ret = cam_mod->custom.s_ext_ctrls(cam_mod, &sp_ctrls);

			kfree(sp_ctrls.ctrls);
		} else
			ret = -ENOMEM;
	...
	struct v4l2_control *ctrl)
{
	...
	return sp_camera_module_s_ext_ctrls(sd, &ext_ctrls);
}
	...
	void *arg)
{
	...
	if (cmd == INTEL_VIDIOC_SENSOR_MODE_DATA) {
		...
		timings->sensor_output_width = sp_timings.sensor_output_width;
		timings->sensor_output_height = sp_timings.sensor_output_height;
		timings->crop_horizontal_start =
			sp_timings.crop_horizontal_start;
		timings->crop_vertical_start = sp_timings.crop_vertical_start;
		timings->crop_horizontal_end = sp_timings.crop_horizontal_end;
		timings->crop_vertical_end = sp_timings.crop_vertical_end;
		timings->line_length_pck = sp_timings.line_length_pck;
		timings->frame_length_lines = sp_timings.frame_length_lines;
		timings->vt_pix_clk_freq_hz = sp_timings.vt_pix_clk_freq_hz;
		timings->binning_factor_x = sp_timings.binning_factor_x;
		timings->binning_factor_y = sp_timings.binning_factor_y;
		timings->coarse_integration_time_max_margin =
			sp_timings.coarse_integration_time_max_margin;
		timings->coarse_integration_time_min =
			sp_timings.coarse_integration_time_min;
		timings->fine_integration_time_max_margin =
			sp_timings.fine_integration_time_max_margin;
		timings->fine_integration_time_min =
			sp_timings.fine_integration_time_min;
...
int sp_camera_module_write_reglist(
	...
	int len)
{
	return pltfrm_superpix_camera_module_write_reglist(&cam_mod->sd, reglist, len);
}
...
int sp_camera_module_write_reg(
	...
	u8 val)
{
	return pltfrm_superpix_camera_module_write_reg(&cam_mod->sd,reg, val);
}
...
int sp_camera_module_read_reg(
	...
	u8 *val)
{
	return pltfrm_superpix_camera_module_read_reg(&cam_mod->sd,
		data_length, reg, val);
}
...
int sp_camera_module_init(struct sp_camera_module *cam_mod,
	struct sp_camera_module_custom_config *custom)
{
	...
	pltfrm_camera_module_pr_info(&cam_mod->sd, "sp_camera_module_init\n");
	...
	pltfrm_camera_module_pr_info(&cam_mod->sd, "sp_camera_module_init ok !\n");
...
void sp_camera_module_release(struct sp_camera_module *cam_mod)
{
	pltfrm_camera_module_pr_debug(&cam_mod->sd, "\n");
