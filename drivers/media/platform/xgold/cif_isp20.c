	...
	struct cif_isp20_device *dev)
{
	...
	u32 i;
	u32 camera_index = 0;
	bool camera_attached = false; /* xuhf@rock-chips.com: v1.0.4 */
	...
	for (i = 0; i < CIF_ISP20_NUM_CSI_INPUTS; i++) {
		...
		if (IS_ERR_OR_NULL(img_src_device)) {
#if 0 /* xuhf@rock-chips.com: v1.0.4 */
		    ...
		    /* xuhf@rock-chips.com: v1.0.4 */
			dev->img_src_array[camera_index] =
				cif_isp20_img_src_to_img_src(img_src_device);
			if (IS_ERR_OR_NULL(dev->img_src_array[camera_index])) {
				dev->img_src_array[camera_index] = NULL;
				ret = -EFAULT;
				goto err;
			}
			cif_isp20_pltfrm_pr_info(dev->dev,
				"device %s attached to CIF CSI-%d,camera_index %d\n",
				cif_isp20_pltfrm_dev_string(img_src_device),
				i, camera_index);
	...
	if (camera_attached) /* xuhf@rock-chips.com: v1.0.4 */
		return 0;
	else
		ret = -EFAULT;
err:
	...
	struct cif_isp20_device *dev)
{
	...
	if (!dev->sp_stream.updt_cfg &&
		...
		return 0;
	/* should ues config.input_sel,
	not the really mipi_config.input_sel,zyc*/
	ret = cif_isp20_pltfrm_g_csi_config(dev->dev,
		/*dev->config.mipi_config.input_sel*/dev->config.input_sel ?
			CIF_ISP20_INP_CSI_1 : CIF_ISP20_INP_CSI_0,
		&dev->config.img_src_output,
		&dev->config.mipi_config.csi_config); /* xuhf@rock-chips.com: v1.0.4 */
	...
	dev->config.mipi_config.input_sel =
			dev->config.mipi_config.csi_config.used_csi;
...
static int cif_isp20_config_mi_mp_frm_size_sync(
	struct cif_isp20_device *dev)
{
	...
	dev->config.mi_config.mp.llength =
			cif_isp20_calc_llength(
				dev->config.mi_config.mp.output.width,
				dev->config.mi_config.mp.output.stride,
				dev->config.mi_config.mp.output.pix_fmt);
	...
	/*cif_iowrite32OR_verify(CIF_MI_MP_FRAME,
		dev->config.base_addr +
		CIF_MI_IMSC, ~0);*/ /* xuhf@rock-chips.com: v1.0.6 */
/*
	cif_iowrite32_verify(0x303030,
		dev->config.base_addr + CIF_MI_QOS_WRITE_MP, 0x3f3f3f);
*/
...
/* xuhf@rock-chips.com: v1.0.6 */
static int cif_isp20_config_mi_mp_sync(
	struct cif_isp20_device *dev,
	u32 save_out_width,
	u32 save_out_height)
{
	...
	u32 bpp = CIF_ISP20_PIX_FMT_GET_BPP(out_pix_fmt);
	u32 size = llength * height * bpp / 8;
	u32 size_original = llength_original * save_out_height * bpp/8;
	u32 mi_ctrl;
	...
	dev->config.mi_config.mp.y_size = size;
	u32 y_size_original = size_original;
	u32 cb_size_original = 0;
	dev->config.mi_config.mp.cb_size = 0;
	dev->config.mi_config.mp.cr_size = 0;
	if (CIF_ISP20_PIX_FMT_IS_YUV(out_pix_fmt)) {
		u32 num_cplanes =
		...
		writeformat = CIF_ISP20_BUFF_FMT_SEMIPLANAR;
		dev->config.mi_config.mp.y_size = width * height;
		y_size_original = save_out_width * save_out_height;
		dev->config.mi_config.mp.cb_size =
			dev->config.mi_config.mp.y_size;
		cb_size_original =
			y_size_original;
	...
	u32 save_out_height)
{
    ...
    /* xuhf@rock-chips.com: v1.0.6 */
    if (dev->otf_zsl_mode == 1 && need_zoom)
        ret = cif_isp20_config_mi_mp_sync(dev,
            save_out_width, save_out_height);
    else
...
static int cif_isp20_config_mi_mp_frm_size(
	struct cif_isp20_device *dev)
{
	int ret = 0;
	...
	ret = cif_isp20_config_mi_mp_frm_size_sync(dev);
	if (IS_ERR_VALUE(ret))
		goto err;
	...
	u32 stream_ids)
{
    ...
    u32 save_out_width = 0;
    u32 save_out_height = 0;
	...
	if ((stream_ids & CIF_ISP20_STREAM_MP) ||
       ...
       /* xuhf@rock-chips.com: v1.0.6 */
       save_out_width = dev->config.mi_config.mp.output.width;
       save_out_height = dev->config.mi_config.mp.output.height;

       if (dev->otf_zsl_mode == 1  && (dev->config.mi_config.mp.output.width !=
           dev->config.isp_config.ism_config.ism_params.h_size)) {
           dev->config.mi_config.mp.output.width =
               dev->config.isp_config.ism_config.ism_params.h_size;
           dev->config.mi_config.mp.output.height =
        ...
        if (stream_ids & CIF_ISP20_STREAM_MP) {
           ret = cif_isp20_config_mp(dev, need_zoom,
               save_out_width, save_out_height);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
	...
	struct cif_isp20_device *dev)
{
	...
	if ((dev->config.mp_config.rsz_config.ycflt_adjust ||
		...
		if (dev->otf_zsl_mode == 1) {
			/*when configure MRSZ, change the mi_config.mp.output
			 so that the scaler will not do dimension scaling.*/
			dev->config.mi_config.mp.output.width =
		dev->config.isp_config.ism_config.ism_params.h_size;
			dev->config.mi_config.mp.output.height =
		dev->config.isp_config.ism_config.ism_params.v_size;
	...
	if (dev->otf_zsl_mode == 1) {
		/*Update the MI pic size of MP only,
		and it is a sync update by SW */
		cif_isp20_config_mi_mp_frm_size(dev);

	if (dev->zoom_info.zoom_buff_info) {
		int i;
		u32 next_buff = cif_ioread32(dev->config.base_addr +
			 CIF_MI_MP_Y_BASE_AD_SHD);
		for (i = 0; i <  dev->zoom_info.max_index; i++) {
			if (next_buff ==
				dev->zoom_info.zoom_buff_info[i].buff_addr) {
				dev->zoom_info.zoom_buff_info[i].width =
				dev->config.mi_config.mp.output.width;
				dev->zoom_info.zoom_buff_info[i].height =
				dev->config.mi_config.mp.output.height;
				dev->zoom_info.cur_index = i;
				break;
			}
		}
	}
...
static int cif_isp20_mi_isr(void *cntxt)
{
	...
	if (!CIF_ISP20_MI_IS_BUSY(dev) &&
		...
		if (dev->sp_stream.state == CIF_ISP20_STATE_STREAMING) {
			spin_lock(&dev->vbq_lock);  /* ddl@rock-chips.com: v1.0.0 */
	...
	int val)
{
	...
	case CIF_ISP20_CID_FLASH_MODE:
		...
		/*xuhf modify in 20150518*/
		/*if (dev->img_src_state == CIF_ISP20_IMG_SRC_STATE_STREAMING)*/
		cif_isp20_img_src_s_ctrl(
			dev->img_src,
			CIF_ISP20_CID_FLASH_MODE,
			dev->config.flash_mode);
		break;
...
int marvin_mipi_isr(void *cntxt)
{
	...
	if ((mipi_mis & CIF_MIPI_SYNC_FIFO_OVFLW(3))) {
		...
		cif_iowrite32OR(CIF_MIPI_CTRL_OUTPUT_ENA,
				dev->config.base_addr + CIF_MIPI_CTRL);
		for (i = 0; i < (sizeof(marvin_hw_errors) / sizeof(struct smarvin_hw_errors)); i++) {
			if (marvin_hw_errors[i].type != 2)
				continue;	/*skip if not mipi error*/

			if (marvin_hw_errors[i].mask & (mipi_mis & CIF_MIPI_SYNC_FIFO_OVFLW(3))) {
		...
		cif_iowrite32(CIF_MI_INIT_SOFT_UPD, dev->config.base_addr + CIF_MI_INIT);
		cif_iowrite32AND(~(CIF_ISP_CTRL_ISP_INFORM_ENABLE |
