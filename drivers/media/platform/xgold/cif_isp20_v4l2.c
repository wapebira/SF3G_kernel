	...
	struct v4l2_buffer *buf)
{
	...
	if ((strm == CIF_ISP20_STREAM_MP)
		&& (dev->zoom_info.zoom_buff_info)) {
		if ((buf->index < dev->zoom_info.max_index)
			&& (dev->zoom_info.zoom_buff_info[buf->index].buff_addr
				== 0)) {
			vbuf = queue->bufs[buf->index];
			dev->zoom_info.zoom_buff_info[buf->index].buff_addr =
	...
	struct v4l2_buffer *buf)
{
	...
	if ((strm == CIF_ISP20_STREAM_MP)
		...
		if (buf->index < dev->zoom_info.max_index) {
			buf->reserved =
				dev->zoom_info.zoom_buff_info[buf->index].width;
			buf->reserved2 =
			dev->zoom_info.zoom_buff_info[buf->index].height;
		}
	}
	...
	struct v4l2_requestbuffers *req)
{
	struct cif_isp20_v4l2_fh *fh = to_fh(file);
	struct cif_isp20_v4l2_node *node = to_node(fh);
	struct cif_isp20_zoom_info zoom_info;
	int ret;
	struct videobuf_queue *queue = to_videobuf_queue(file);
	struct cif_isp20_device *dev = to_cif_isp20_device(queue);
	enum cif_isp20_stream_id strm = to_cif_isp20_stream_id(queue->type);
	...
	if (strm == CIF_ISP20_STREAM_MP) {
		kfree(dev->zoom_info.zoom_buff_info);
		dev->zoom_info.zoom_buff_info = NULL;
		zoom_info.zoom_buff_info = kmalloc(
		req->count * sizeof(struct cif_isp20_zoom_buffer_info),
			GFP_KERNEL);

		if (!zoom_info.zoom_buff_info) {
			cif_isp20_pltfrm_pr_err(NULL,
				"zoom_info.zoom_buff_info NULL %d\n", ret);
			return -ENOMEM;
		} else {
			for (i = 0; i < req->count; i++) {
				zoom_info.zoom_buff_info[i].flags = 0;
				zoom_info.zoom_buff_info[i].buff_addr = 0;
				zoom_info.zoom_buff_info[i].height = 0;
				zoom_info.zoom_buff_info[i].width = 0;
			}
			dev->zoom_info.cur_index = 0;
			dev->zoom_info.max_index = req->count;
			dev->zoom_info.zoom_buff_info =
				zoom_info.zoom_buff_info;
		}
	}
	...
	struct file *file)
{
	...
	/*add by xuhf by 20150525*/
	wake_lock(&dev->torch_wake);
	return 0;
err:
	wake_unlock(&dev->torch_wake);
	cif_isp20_pltfrm_pr_err(NULL,
		"failed with error %d\n", ret);
	return ret;
}
...
/* ddl@rock-chips.com: add */
static long v4l2_default_ioctl_rk(struct file *file, void *fh,
			       bool valid_prio, unsigned int cmd, void *arg)
{
	...
	if (cmd == RK_VIDIOC_CAMERA_MODULEINFO) {
		struct rk_xgold_camera_module_info_s *p_camera_module =
		(struct rk_xgold_camera_module_info_s *)arg;
			       ...
			       bool valid_prio, unsigned int cmd, void *arg)
{
	...
	} else if (cmd == INTEL_VIDIOC_ZOOM_W_H) {
		struct xgold_zoom_buff *p_data =
		(struct xgold_zoom_buff *)arg;
		int ret;
		struct videobuf_queue *queue = to_videobuf_queue(file);
		enum cif_isp20_stream_id strm =
			to_cif_isp20_stream_id(queue->type);
		struct cif_isp20_device *dev = to_cif_isp20_device(queue);

		if ((strm == CIF_ISP20_STREAM_MP)
			&& (dev->zoom_info.zoom_buff_info)) {
			if (p_data->index < dev->zoom_info.max_index) {
				p_data->width =
			dev->zoom_info.zoom_buff_info[p_data->index].width;
				p_data->height =
			dev->zoom_info.zoom_buff_info[p_data->index].height;
			} else {
				p_data->width = 0;
				p_data->height = 0;
			}
	...
	} else if (cmd == INTEL_VIDIOC_OTF_ZSL_MODE) {
		int otf_zsl_mode = *((int *)arg);
		struct videobuf_queue *queue = to_videobuf_queue(file);
		enum cif_isp20_stream_id strm =
			to_cif_isp20_stream_id(queue->type);
		struct cif_isp20_device *dev = to_cif_isp20_device(queue);
		if (strm == CIF_ISP20_STREAM_MP)
			dev->otf_zsl_mode = otf_zsl_mode;
	} else {
		/* ddl@rock-chips.com: add */
		ret = v4l2_default_ioctl_rk(file, fh, valid_prio, cmd, arg);
		if (ret < 0)
			cif_isp20_pltfrm_pr_err(dev->dev,
...
extern int register_mipitune_device(struct cif_isp20_device *xgold_v4l2);
static int xgold_v4l2_drv_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto err;
	register_mipitune_device(dev);
	wake_lock_init(&dev->torch_wake, WAKE_LOCK_SUSPEND, "cif_torch_wake");
...
static int xgold_v4l2_drv_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&cif_isp20_dev->torch_wake);
...
static int xgold_v4l2_drv_resume(struct platform_device *pdev)
{
	...
	if (!cif_isp20_dev->img_src) {
		cif_isp20_pltfrm_pr_err(
			cif_isp20_dev->dev,
			"cif_isp20_dev img_src is null!\n");
		goto err;
	}
