...
/* xuhf@rock-chips.com: v1.0.4 */
	...
	if (IS_ERR_VALUE(ret)) {
		cif_isp20_pltfrm_pr_err(dev,
			"reading property 'intel,used_csi'\n");
		goto err;
	}
#else
	{
		struct device_node *parent_node =
						of_get_parent(csi_config_node);

		if (parent_node != NULL) {
			const char *full_name = of_node_full_name(parent_node);

			if (full_name != NULL) {
				int ret = kstrtoint(
						full_name+strlen(full_name) - 1,
						10, &csi_config->used_csi);
				if (ret < 0)
					cif_isp20_pltfrm_pr_err(dev,
					"reading property 'intel,csi-dphy2'\n");
			}
	...
	enum cif_isp20_inp inp)
{
	...
	int num_cameras = 0; /* xuhf@rock-chips.com: v1.0.4 */
	...
	if ((inp == CIF_ISP20_INP_CSI_0) ||
		...
		for (index = 0; index < size/sizeof(phandle); index++) {
			...
			if (!strcmp(camera_list_node->type,
				...
				continue; /* xuhf@rock-chips.com: v1.0.5 */
#if 0
					ret = -EPROBE_DEFER;
					goto err;
#endif
				...
				of_node_put(camera_list_node);
				continue; /* xuhf@rock-chips.com: v1.0.4 */
#if 0
					ret = -EINVAL;
					goto err;
#endif
	...
	if(!find_camera) /* xuhf@rock-chips.com: v1.0.5 */
		goto err;

	cif_isp20_pltfrm_pr_dbg(dev,
		"found device %s attached to CIF CSI-%d",
		dev_driver_string(&client->dev), index);
	if (find_camera) /* xuhf@rock-chips.com: v1.0.4 */
		return &client->dev;
err:
