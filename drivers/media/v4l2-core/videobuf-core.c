...
checks:
	...
	if (list_empty(&q->stream)) {
		if (noblock) {
			...
			printk(KERN_ERR "vbuf: next_buffer: no buffers to dequeue\n");
			goto done;
		} else {
		   ...
		   struct v4l2_buffer *b, int nonblocking)
{
	...
	if (retval < 0) {
		...
		printk(KERN_DEBUG "dqbuf: next_buffer error: %i\n", retval);
		goto done;
	}
	...
	case VIDEOBUF_ERROR:
		...
		printk(KERN_DEBUG "dqbuf: state is error\n");
		break;
	case VIDEOBUF_DONE:
	...
	default:
		...
		printk(KERN_DEBUG "dqbuf: state invalid\n");
		retval = -EINVAL;
		goto done;
	}
	CALL(q, sync, q, buf);
				  ...
				  poll_table *wait)
{
	...
	if (!buf) {
		rc = POLLERR;
		printk(KERN_ERR "vbuf: no buf, videobuf_poll_stream failed!\n");
	}
