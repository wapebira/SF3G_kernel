...
#define DISCHRG_TIME_STEP_0	(10 * 60)
...
struct rk81x_battery {
	...
	unsigned long			suspend_time_end;
...
u32 support_loader_chrg, support_usb_adp, support_dc_adp;
...
static bool rk81x_bat_support_loader_chrg(void)
{
	return support_loader_chrg ? true : false;
}
				      ...
				      unsigned long event, void *data)
{
	...
	case USB_EVENT_CHARGER:
		...
		wake_lock_timeout(&di->resume_wake_lock, 3 * HZ);
...
static void rk81x_bat_not_first_pwron(struct rk81x_battery *di)
{
	 ...
	 */
	if (rk81x_bat_support_loader_chrg())
		goto out;
...
static void rk81x_bat_low_waring_init(struct rk81x_battery *di)
{
	u8 vb_mon_reg;
	u8 vb_mon_reg_init;

	rk81x_bat_read(di, VB_MOD_REG, &vb_mon_reg, 1);

	/* 3.4v: interrupt*/
	vb_mon_reg_init = (((vb_mon_reg | (1 << 4)) & (~0x07)) | 0x06);
	rk81x_bat_write(di, VB_MOD_REG, &vb_mon_reg_init, 1);
}
...
static void rk81x_bat_zero_calc_linek(struct rk81x_battery *di)
{
	...
	if (count_num >  10) {
		for (i = 0; i < 9; i++)
			vol_fifo[i] = vol_fifo[i + 1];
		count_num = 10;
	}
	vol_fifo[count_num - 1] = di->voltage;

	for (i = 0; i < count_num; i++)
		avg_voltage += vol_fifo[i];

	avg_voltage /= count_num;

	DBG("ZERO0: %d %d %d %d %d %d %d %d %d %d / %d\n",
	    vol_fifo[0], vol_fifo[1], vol_fifo[2], vol_fifo[3],
	    vol_fifo[4], vol_fifo[5], vol_fifo[6], vol_fifo[7],
	    vol_fifo[8], vol_fifo[9], avg_voltage);
	...
	ocv_voltage = avg_voltage - (currentnow * di->bat_res) / 1000;
	...
	di->zero_avl_soc = ocv_soc - dead_soc;
	if ((di->dsoc == 1) && (di->zero_avl_soc > 0)) {/*discharge*/
		di->line_k = 1000;
	} else if (di->zero_avl_soc > 1) {
		di->line_k = (di->display_soc + di->zero_avl_soc / 2) / div(di->zero_avl_soc);
	} else {
		di->dsoc--;
		di->display_soc = di->dsoc * 1000;
	}

	if (ocv_voltage <= dead_voltage) {
		di->dsoc >>= 1;
		di->display_soc = di->dsoc * 1000;
		dev_warn(di->dev, "battery voltage low\n");
	}

	di->zero_old_remain_cap = di->remain_capacity;

	DBG("ZERO-new: new-line_k=%d, dsoc=%d, available_soc =%d\n"
	    "ZERO-new: di->display_soc=%d, old_remain_cap=%d\n\n",
	    di->line_k, di->dsoc, di->zero_avl_soc,
...
static void rk81x_bat_dischrg_smooth(struct rk81x_battery *di)
{
	...
	if (di->dischrg_algorithm_mode == DISCHRG_NORMAL_MODE) {
		...
		if (di->voltage < ZERO_ALGOR_THRESD || di->dsoc < 15) {
			di->dischrg_normal_base = 0;
			di->chrg_emu_base = 0;
		...
		if (di->voltage > ZERO_ALGOR_THRESD + 50 && di->dsoc >= 15) {
			di->dischrg_algorithm_mode = DISCHRG_NORMAL_MODE;
			di->zero_timeout_cnt = 0;
...
static void rk81x_bat_dbg_dmp_info(struct rk81x_battery *di)
{
	...
	DBG("#######################################################\n"
	    ...
	    "emu_dischrg = %lu, power_on_time = %lu\n"
	    ...
	    BASE_TO_MIN(di->power_on_base),
	    di->current_mode, di->chrg_save_sec,
	    di->dischrg_save_sec);
}
...
static void disable_vbat_low_irq(struct rk81x_battery *di)
{
	/* mask vbat low */
...
static void enable_vbat_low_irq(struct rk81x_battery *di)
{
	/* clr vbat low interrupt */
	rk818_set_bits(di->rk818, 0x4c, (0x1 << 1), (0x1 << 1));
	/* mask vbat low */
...
/* plug_in_irq and plug_out_irq used in rk818.c */
#if defined(CONFIG_X86_INTEL_SOFIA)
				  ...
				  struct device_node *np)
{
	...
	di->pins_default = pinctrl_lookup_state(di->pinctrl, "dc_det");
...
static int rk81x_bat_parse_dt(struct rk81x_battery *di)
{
	...
	ret = of_property_read_u32(np, "support_loader_chrg", &support_loader_chrg);
	...
	DBG("\nthe battery dts info dump:\n"
	    ...
	    cell_cfg->ocv->sleep_exit_current, support_loader_chrg,
...
static int rk81x_battery_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(di->wq, &di->battery_monitor_work,
			   msecs_to_jiffies(TIMER_MS_COUNTS * 50));
	if (0) {
				 ...
				 pm_message_t state)
{
	...
	di->chrg_status = rk81x_bat_get_chrg_status(di);
	di->suspend_time_start = get_runtime_sec();
	...
	DBG("<%s>. suspend_time_start:%lld\n",
	    __func__, get_runtime_sec());
...
static int rk81x_battery_resume(struct platform_device *dev)
{
	...
	di->est_ocv_soc = rk81x_bat_est_ocv_soc(di);
	di->remain_capacity = rk81x_bat_get_realtime_capacity(di);
	di->suspend_time_end = get_runtime_sec();
	delta_time = di->suspend_time_end - di->suspend_time_start;
	...
	if ((!rk81x_chrg_online(di) && di->voltage <= pwroff_thresd) ||
		...
		wake_lock_timeout(&di->resume_wake_lock, 50 * HZ);
