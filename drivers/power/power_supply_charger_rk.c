...
{
	psc_dbg_printk(
		"%s:name=%s present=%d is_charging=%d health=%d online=%d cable=%lu tstamp=%lu\n",
...
{
	psc_dbg_printk(
		"%s:name=%s voltage_now=%ld current_now=%ld temperature=%d status=%ld health=%d tstamp=%lu algo_stat=%d ",
