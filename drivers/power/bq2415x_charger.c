...
#define BQ2415X_MASK_SAFETY_LIMIT	(BIT(4)|BIT(5)|BIT(6)|BIT(7))
#define BQ2415X_SHIFT_SAFETY_LIMI	4
...
static int bq2415x_set_current_limit(struct bq2415x_device *bq, int mA)
{
	...
	if (mA <= 100){
		val = 0;
		bq2415x_i2c_write_bit(bq, BQ2415X_REG_SPECIAL,
				1, 5);
	}
	else if (mA <= 500)
	{
...
static int bq2415x_set_safety_limit_current(struct bq2415x_device *bq, int mA)
{
	int val;
	...
	if (val < 0)
		...
		val = 7;
	return bq2415x_i2c_write_mask(bq, BQ2415X_REG_LIMIT, val,
			BQ2415X_MASK_SAFETY_LIMIT,
			BQ2415X_SHIFT_SAFETY_LIMI);
}
...
{
	bq2415x_set_safety_limit_current(bq, bq->init_data.charge_current); /* Writed only once after reset */
		...
		unsigned long val, void *v)
{
	...
	ret = psy->get_property(psy, POWER_SUPPLY_PROP_USB_OTG, &prop);
	if (ret != 0)
		return NOTIFY_OK;

	boot_otg = prop.intval;
...
/*The work will be instant response power notify some operations*/
static void bq2415x_notify_work(struct work_struct *work)
{
	struct bq2415x_device *bq = container_of(work, struct bq2415x_device,
						 notify_work);
	if (bq->reported_mode != bq->mode) {
		sysfs_notify(&bq->charger.dev->kobj, NULL, "reported_mode");
		bq2415x_set_mode(bq, bq->reported_mode);
	}
	bq2415x_exec_command(bq, BQ2415X_TIMER_RESET);
...
static int bq2415x_power_supply_init(struct bq2415x_device *bq)
{
	...
	bq->charger.supplied_to = chrg_supplied_to;
	bq->charger.num_supplicants = ARRAY_SIZE(chrg_supplied_to);
			 ...
			 const struct i2c_device_id *id)
{
	...
	INIT_WORK(&bq->notify_work, bq2415x_notify_work);
	bq2415x_set_autotimer(bq, 1);
