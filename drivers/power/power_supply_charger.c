...
{
	int ret = 0;
	struct device_node *fg_node = NULL;
	struct device_node *np = psy->dev->parent->of_node;

	fg_node = of_parse_phandle(np, "sofia,fuel_gauge", 0);
	if (!psy_chrgr.is_cable_evt_reg && of_device_is_available(fg_node)) {
