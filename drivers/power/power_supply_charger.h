...
#define DEF_CUR_VOLT_SAMPLE_JIFF (1*HZ)
...
#define enable_charging(psy) \
		...
		set_ps_int_property(psy, POWER_SUPPLY_PROP_ENABLE_CHARGING,\
					true); } \
		continue_charging(psy); })
#define disable_charging(psy) \
		set_ps_int_property(psy,\
				POWER_SUPPLY_PROP_ENABLE_CHARGING, false);

#define continue_charging(psy) \
		set_ps_int_property(psy, \
			POWER_SUPPLY_PROP_CONTINUE_CHARGING, true)
...
#define SET_MAX_TEMP(psy, temp) \
		set_ps_int_property(psy,\
				POWER_SUPPLY_PROP_TEMP_ALERT_MAX, temp)
#define SET_MIN_TEMP(psy, temp) \
		set_ps_int_property(psy,\
				POWER_SUPPLY_PROP_TEMP_ALERT_MIN, temp)
...
#define IS_CHARGER(psy) (psy->type == POWER_SUPPLY_TYPE_MAINS ||\
...
#define IS_BAT_PROP_CHANGED(bat_prop, bat_cache)\
	...
	(time_after(bat_prop.tstamp, (bat_cache.tstamp + PROP_TTL)) &&\
