...
/* The estimate resistance of battery*/
#define BAT_RESISTANCE_MILLIOHM			(150)
/* The current from battery in loader*/
#define BAT_CURRENT_IN_LOADER_MA		(200)
/* The voltage of low power threshold*/
#define OCV_VOLTAGE_LOW_POWER_THRESHOLD		(3700)
/* The offset of OCV capacity in low power */
#define OCV_CAPACITY_LOW_POWER_OFFSET		(50)
/*
...
#define SCALE_IBAT_NOW_RELAXED			(SCALE_PERCENT / 5)
...
static void sw_fuel_gauge_set_capacity(int capacity_permil)
{
	...
	iio_read_channel_processed(sw_fuel_gauge_instance.vbat.p_iio_vbat_typ,
				   &iio_vbat_mv);
	...
	if ((p_properties->internal_capacity != capacity_percent) ||
		...
		if (p_properties->status == POWER_SUPPLY_STATUS_CHARGING ||
			...
			if (p_properties->reported_capacity >
				...
				p_properties->reported_capacity =
					p_properties->internal_capacity;
			if (iio_vbat_mv < sw_fg->bat.p_fitted_model->cap_to_vbat_ocv[4] && p_properties->reported_capacity > 1 && p_properties->internal_capacity < 15) {
				offset_cc = sw_fg->nom_bat_capacity_mc * 3 / 1000;
				sw_fg->last_soc_cal.cc_balanced_mc -= offset_cc;
			}
		}

		p_properties->capacity_valid = true;

		p_properties->init_first_time = 0;
		...
		SW_FUEL_GAUGE_DEBUG_PARAM(
			SW_FUEL_GAUGE_DEBUG_SET_PROPERTY_CAPACITY,
							capacity_percent);
		if (p_properties->reported_capacity == 0)
...
static void sw_fuel_gauge_tbat_monitor(void)
{
	...
	if ((-EOVERFLOW == tbat_error) && (BAT_ID_PSU ==
		...
		case IIO_VAL_INT:
			...
			POWER_SUPPLY_PROP_TEMP */
			if (sw_fuel_gauge_instance.properties.init_first_time != 1) {
				tbat_diff_value = tbat_value - sw_fuel_gauge_instance.last_tbat;
				if (tbat_diff_value > 16 || tbat_diff_value < -16)
					tbat_value = sw_fuel_gauge_instance.last_tbat;
			}
			sw_fuel_gauge_set_temperature(tbat_value, true);
			sw_fuel_gauge_instance.last_tbat = tbat_value;
			/*
...
static void sw_fuel_gauge_calc_battery_values(void)
{
	...
	sw_fuel_gauge_instance.ibat_load_avg_relaxed_limit_ma =
				(nom_bat_capacity_mah / SCALE_IBAT_AVG_RELAXED) + 50;
...
static void sw_fuel_gauge_estimate_initial_capacity_and_error(void)
{
	...
	measurement. */
	if (sw_fuel_gauge_instance.loader_bat_ocv_mv != 0) {
		ocv_mv = sw_fuel_gauge_instance.loader_bat_ocv_mv +
			(BAT_RESISTANCE_MILLIOHM * BAT_CURRENT_IN_LOADER_MA) / 1000;
	} else {
	/* Can not get the voltage from loader */
		ocv_mv = sw_fuel_gauge_instance.vbat.vbat_typ_mv;
	}
	sw_fuel_gauge_calc_ocv_capacity_and_error(ocv_mv,
		&sw_fuel_gauge_instance.latest_calculated_capacity_permil,
			&sw_fuel_gauge_instance.latest_calculated_error_permil);

	if (sw_fuel_gauge_instance.loader_bat_ocv_mv < OCV_VOLTAGE_LOW_POWER_THRESHOLD &&
	    sw_fuel_gauge_instance.latest_calculated_capacity_permil > OCV_CAPACITY_LOW_POWER_OFFSET) {
		sw_fuel_gauge_instance.latest_calculated_capacity_permil -= OCV_CAPACITY_LOW_POWER_OFFSET;
	}
...
static void sw_fuel_gauge_calculate_nvm_capacity_and_error(void)
{
	...
	if (sw_fuel_gauge_instance.nvm_valid_at_init) {
		...
		printk("%s: We have retrieved a valid calibration point from NVM\n",__func__);
		sw_fuel_gauge_instance.latest_calculated_capacity_permil =
		...
		sw_fuel_gauge_instance.latest_calculated_error_permil =
					p_last_soc_cal->soc_error_permil;
		if (sw_fuel_gauge_instance.latest_calculated_capacity_permil < 10)
			sw_fuel_gauge_instance.latest_calculated_capacity_permil += 10;
	} else {
 ...
 */
static int sw_fuel_gauge_probe(struct platform_device *p_platform_dev)
{
	...
	sw_fuel_gauge_instance.properties.init_first_time = 1;
 ...
 * allocated resources.
 */
static int sw_fuel_gauge_remove(struct platform_device *p_platform_dev)
{
	/* Trace and Debug data. */
...
static int __init fuel_gauge_loader_voltage_setup(char *str)
{
	int volt;

	volt = simple_strtoul(str, NULL, 10);
	if (volt != 0)
		sw_fuel_gauge_instance.loader_bat_ocv_mv = volt;
	return 0;
}
__setup("loader_voltage=", fuel_gauge_loader_voltage_setup);
...
static int sw_fuel_gauge_init(void)
{
	/*
	 * Initialise spinlock.
...
static void sw_fuel_gauge_exit(void)
{
