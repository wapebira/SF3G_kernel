...
/*
   currently system always halt and k2u fifo keep full in deepsleep mode.
   so this is only workaround for deepsleep mode. we can revert back it
   once we have better solution 
*/
#define DEEPSLEEP_WORKAROUND 1
...
int fmdev_send_vector(struct iovec *vector, const size_t count)
{
	...
	if (kfifo_avail(&(fmdev_info.k2u_fifo.fifo)) < total_size) {
		pr_crit("fmdev_send_vector(count: %u): Not enough space in fifo!\n",
					count);
		retval = -EAGAIN;
	}
