...
static int imc_debug_add_ctrl(struct idi_controller_device *idi)
{
	struct imc_controller *imc_idi = idi_controller_get_drvdata(idi);
	struct dentry *dir;
...
__acquires(&imc_idi->sw_lock) __releases(&imc_idi->sw_lock)
{
	...
	while (node != queue) {
		...
		trans->current_queue = IDI_INVALID_QUEUE;
...
__acquires(&imc_idi->sw_lock) __releases(&imc_idi->sw_lock)
{
	...
	if (trans->current_queue == IDI_XFER_QUEUE ||
			trans->current_queue == IDI_FWD_QUEUE){
		dev_err(imc_idi->dev, "Transaction %p is already queued\n",
		trans);
		return -EBUSY;
	}
	/*
	...
	trans->current_queue = IDI_XFER_QUEUE;
 ...
 */
#define IDIDBB_MULT_SRC_SRQ_NUM  89
static int imc_controller_request_irqs(struct imc_controller *imc_idi)
{
	...
	for (i = 0; i < IMC_IDI_MAX_IRQS; i++) {
		if (imc_idi->irqs[i] == -1)
			return 0;
		if (imc_idi->irqs[i] == 82)
			continue;

		if (IDIDBB_MULT_SRC_SRQ_NUM == imc_idi->irqs[i])
...
struct imc_controller *imc_alloc_controller(struct device *dev)
{
	...
	memset(imc_idi->irqs, -1, sizeof(imc_idi->irqs));
