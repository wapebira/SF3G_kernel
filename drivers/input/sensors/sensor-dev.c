		...
		struct pinctrl_state *state)
{
	...
	int ret = 0;
	if (!IS_ERR(state)) {
		ret = pinctrl_select_state(sensor->pdata->pinctrl, state);
		if (ret)
			dev_err(&client->dev, "%d:could not set pins\n", __LINE__);
	}
...
static int sensor_get_id(struct i2c_client *client, int *value)
{
	struct sensor_private_data *sensor =
	    (struct sensor_private_data *) i2c_get_clientdata(client);	
	int result = 0;
	unsigned char temp = sensor->ops->id_reg;
	int i = 0;
	...
	if(sensor->ops->id_reg >= 0)
	{
		for(i=0; i<3; i++)
		{
			if ((sensor->ops->id_i2c == LIGHT_ID_CM36682) || (sensor->ops->id_i2c == PROXIMITY_ID_CM36682)) {
				*value = sensor_read_word_reg(client, temp);
				if (*value == 0x83)
					break;
			} else {
...
static irqreturn_t sensor_interrupt(int irq, void *dev_id)
{
	...
	if (sensor->pdata->type == SENSOR_TYPE_PROXIMITY)
		wake_lock_timeout(&sensor->suspend_lock, 1*HZ);
	//use threaded IRQ
...
static int sensor_irq_init(struct i2c_client *client)
{
	...
	if ((sensor->pdata->irq_enable) && (sensor->ops->trig != SENSOR_UNKNOW_DATA))
	{
		...
		if(sensor->pdata->poll_delay_ms < 0)
			sensor->pdata->poll_delay_ms = 200;
		printk("sensor_irq_init--------client->irq=%d\n",client->irq);
		...
		result = devm_request_threaded_irq(&client->dev, client->irq, NULL, sensor_interrupt, sensor->ops->trig, sensor->ops->name, sensor);
		if (result) {
			printk(KERN_ERR "%s:fail to request irq = %d, ret = 0x%x\n",__func__, client->irq, result);	       
			goto error;	       
		}
		if (sensor->pdata->type == SENSOR_TYPE_ANGLE)
			disable_irq_nosync(client->irq);/*disable irq*/
		if (((sensor->pdata->type == SENSOR_TYPE_GYROSCOPE) || (sensor->pdata->type == SENSOR_TYPE_ACCEL)) && (!(sensor->ops->trig & IRQF_SHARED)))
			disable_irq_nosync(client->irq);/*disable irq*/
...
static void sensor_suspend(struct early_suspend *h)
{
	...
	if (sensor->pins_sleep)
		sensor_set_pinctrl_state(sensor->client,
					sensor->pins_sleep);
...
int sensor_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	pdata->project_name = (char* )of_get_property(np, "project_name", NULL);
	...
	sensor_set_pinctrl_state(sensor->client,  pdata->pins_default);
	...
	wake_lock_init(&sensor->suspend_lock,
				WAKE_LOCK_SUSPEND,
				"sensor_wake_lock");
...
static int sensor_remove(struct i2c_client *client)
{
	...
	input_free_device(sensor->input_dev);	
	kfree(sensor);
	sensor_set_pinctrl_state(client, pdata->pins_inactive);
#ifdef CONFIG_HAS_EARLYSUSPEND
	if((sensor->ops->suspend) && (sensor->ops->resume))
		unregister_early_suspend(&sensor->early_suspend);
#endif
	wake_lock_destroy(&sensor->suspend_lock);
	return result;
}
...
static const struct i2c_device_id sensor_id[] = {
	...
	{"lsm303c_mag", COMPASS_ID_LSM303C},
	/*gyroscope*/
	{"gyro", GYRO_ID_ALL},	
	...
	{"ls_tmd2772", LIGHT_ID_TMD2772},
	{"ls_cm36682", LIGHT_ID_CM36682},
	...
	{"ps_tmd2772", PROXIMITY_ID_TMD2772},
	{"ps_cm36682", PROXIMITY_ID_CM36682},
...
static struct of_device_id sensor_dt_ids[] = {
	...
	{ .compatible = "ls_tmd2772" },
	{ .compatible = "capella,ls_cm36682" },
	...
	/*proximity sensor*/
	{ .compatible = "ps_tmd2772" },
	{ .compatible = "capella,ps_cm36682" },
