...
static int sensor_init(struct i2c_client *client)
{
	...
	if (res) {
		printk("Set Accelerometer Configuration 1(0x1C) error,res: %d!\n", res);
		return res;
	}
	msleep(20);
	...
	if (res) {
		printk("Set Accelerometer Configuration 2 (0x1D) error,res: %d!\n", res);
		return res;
	}
	msleep(20);

	res = sensor_write_reg(client, MPU6880_PWR_MGMT_1, 0x41);  /*The chip is set to sleep mode*/
	if (res) {
		printk("Set Power Management 1 _0x6B_ error,res: %d!\n", res);
		return res;
	}
	msleep(20);

	res = sensor_write_reg(client, MPU6880_INT_ENABLE, 0x01); /*Enable Raw Sensor Data Ready interrupt to propagate to interrupt pin*/
	if (res) {
		printk("Set Interrupt Enable _0x38_ error,res: %d!\n", res);
		return res;
	}
	msleep(20);
	return res;
