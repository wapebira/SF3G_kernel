/* drivers/input/sensors/access/bma2x2.c
 *
...
#define Y_AXIS_COMPEN	350000	/*Y_AXIS offset is caused by screws, there needs to be compensation 3.5*/
...
static int sensor_init(struct i2c_client *client)
{
	...
	if (ret < 0)
		printk(KERN_INFO "set bandwidth failed!\n");

	ret = bma2x2_set_range(client, BMA2X2_RANGE_SET);
	if (ret < 0)
		printk(KERN_INFO "set g-range failed!\n");
	...
	if (sensor->pdata->irq_enable) {
		...
		ret = sensor_write_reg(client, BMA2X2_INT_ENABLE2_REG, 0x10);
		if (ret) {
			dev_err(&client->dev, "interrupt engines register setting failed!\n");
			return ret;
		}

		ret = sensor_write_reg(client, BMA2X2_INT_DATA_SEL_REG, 0x10);
		if (ret) {
...
struct sensor_operate gsensor_bma2x2_ops = {
	...
	.id_data			= BMA222E_CHIP_ID,				/*device id*/
