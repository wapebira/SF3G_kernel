...
#ifdef DEBUG
	printk("%s read x=0x%02x 0x%02x (regH regL), x=%d (dec) [LSB]\n",
		LSM303C_MAG_DEV_NAME, mag_data[1], mag_data[0], hw_d[0]);
	printk("%s read y=0x%02x 0x%02x (regH regL), y=%d (dec) [LSB]\n",
		LSM303C_MAG_DEV_NAME, mag_data[3], mag_data[2], hw_d[1]);
	printk("%s read z=0x%02x 0x%02x (regH regL), z=%d (dec) [LSB]\n",
...
{
	input_report_abs(stat->input_dev_mag, ABS_HAT0X, xyz[0]);
	input_report_abs(stat->input_dev_mag, ABS_HAT0Y, xyz[1]);
	input_report_abs(stat->input_dev_mag, ABS_BRAKE, xyz[2]);
//	 input_report_abs(stat->input_dev_mag, ABS_HAT1X, xyz[2]);
...
static int lsm303c_mag_input_init(struct lsm303c_status *stat)
{
	...
	stat->input_dev_mag->name = "compass";
	...
	input_set_abs_params(stat->input_dev_mag, ABS_HAT0X,
				-MAG_G_MAX_NEG, MAG_G_MAX_POS, FUZZ, FLAT);
	input_set_abs_params(stat->input_dev_mag, ABS_HAT0Y,
				-MAG_G_MAX_NEG, MAG_G_MAX_POS, FUZZ, FLAT);
	input_set_abs_params(stat->input_dev_mag, ABS_BRAKE,
				-MAG_G_MAX_NEG, MAG_G_MAX_POS, FUZZ, FLAT);
	input_set_abs_params(stat->input_dev_mag, ABS_HAT1X,
				-MAG_G_MAX_NEG, MAG_G_MAX_POS, FUZZ, FLAT);
...
static struct lsm303c_status *stat;
...
{
       int result = 0;
	 //  printk("==========fun = %s ,line = %d ,ECS_IOCTL_APP_SET_MVFLAG = 0x%x, arg = 0x%x\n",
	 //  			__FUNCTION__,__LINE__,ECS_IOCTL_APP_SET_MVFLAG, *((short*)arg));
	if(stat != NULL)
	{
    	switch (cmd) {
        case ECS_IOCTL_APP_SET_MVFLAG:
			// printk("==========fun = %s ,line = %d ,\n",__FUNCTION__,__LINE__);
			 if(*((short*)arg) == 1)
			 {
             	result = lsm303c_mag_enable(stat);
			 }else if(*((short*)arg) == 0)
			 {
				result =lsm303c_mag_disable(stat);
			 }else
			 {
				result =0;
			 }
...
static struct miscdevice adc_miscdev = 
{
    ...
    .name    = "compass",        /*<C9>υτ<C3><FB><B3><C6>*/
    .fops    = &adc_fops,          /*<B6><D4>ADC<C9>υτ<CE>?<FE><B2><D9><D7><F7>*/
};
		...
		struct device *dev)
{
	...
	acc_pdata->fs_range = LSM303C_MAG_FS_16G;
