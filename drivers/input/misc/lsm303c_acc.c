...
static struct lsm303c_acc_platform_data *lsm303c_acc_of_get_platdata(
		struct device *dev)
{
	struct device_node *np = dev->of_node;
	struct lsm303c_acc_platform_data *acc_pdata;
	u32 out_values[3];
	int ret = 0;
...
skip_pinctrl:
	...
	acc_pdata->fs_range = LSM303C_ACC_FS_2G;
	acc_pdata->min_interval = LSM303C_ACC_MIN_POLL_PERIOD_MS; /* 2ms */
	acc_pdata->gpio_int1 =
		LSM303C_ACC_DEFAULT_INT1_GPIO;
...
static inline int lsm303c_acc_set_pinctrl_state(struct device *dev,
		struct pinctrl_state *state)
{
	struct lsm303c_acc_status *stat = dev_get_drvdata(dev);
	struct lsm303c_acc_platform_data *pdata = stat->pdata;
	int ret = 0;
						...
						struct lsm303c_acc_status *stat)
{
	...
	if (stat->client->irq > 0 || stat->pdata->gpio_int1 >= 0)
...
//printk("func = %s ,line = %d , stat->use_smbus = %d \n", __FUNCTION__,__LINE__,stat->use_smbus);
	if (stat->use_smbus) {
		if (len == 1) {
...
static int lsm303c_acc_hw_init(struct lsm303c_acc_status *stat)
{
	...
	if (err < 0) {
		dev_warn(&stat->client->dev,
		"Error reading WHO_AM_I: is device available/working? i2c addr %x\n", stat->client->addr);
		goto err_firstread;
	} else
		stat->hw_working = 1;
...
static void lsm303c_acc_device_power_off(struct lsm303c_acc_status *stat)
{
	...
	dev_dbg(&stat->client->dev, "%s: device_power_off\n", LSM303C_ACC_DEV_NAME);
	err = lsm303c_acc_i2c_write(stat, buf, 1);
	if (err < 0)
...
{
	int err = -1;
	dev_dbg(&stat->client->dev, "%s: device_power_on\n", LSM303C_ACC_DEV_NAME);
...
{
	int err;
	dev_dbg(&stat->client->dev, "%s: lsm303c_acc_enable \n", LSM303C_ACC_DEV_NAME);
...
{
	dev_dbg(&stat->client->dev, "%s: lsm303c_acc_disable \n", LSM303C_ACC_DEV_NAME);
...
static struct lsm303c_acc_status *stat;
...
{
	int result = 0;
	DTRACE("lsm303c_acc line=%d cmd=%x GSENSOR_IOCTL_START=%x  GSENSOR_IOCTL_CLOSE=%x\n", __LINE__, cmd, GSENSOR_IOCTL_START, GSENSOR_IOCTL_CLOSE);
 	if(stat != NULL)
 	{
	    switch (cmd) {
			 case GSENSOR_IOCTL_START:
			 	DTRACE("lsm303c GSENSOR_IOCTL_START \n");
			 	result = lsm303c_acc_enable(stat);
			 	break;
			
	         case GSENSOR_IOCTL_CLOSE:
			 	DTRACE("lsm303c GSENSOR_IOCTL_CLOSE \n");
			 	result = lsm303c_acc_disable(stat);
	 			break;
			 default:
                printk("Unknow cmd\n");
                return -EINVAL;
		...
		const struct i2c_device_id *id)
{
	...
	client->dev.platform_data =
		lsm303c_acc_of_get_platdata(&client->dev);
	...
	/* pcl */
	lsm303c_acc_set_pinctrl_state(&client->dev,
	...
	dev_info(&client->dev, "%s: irq = %d\n", LSM303C_ACC_DEV_NAME, stat->irq1);
#endif
	...
	dev_info(&client->dev, "%s: dev_info probed\n", LSM303C_ACC_DEV_NAME);
	pr_debug("%s: pr_debug probed\n", LSM303C_ACC_DEV_NAME);
	dev_dbg(&client->dev, "%s: dev_dbg probed\n", LSM303C_ACC_DEV_NAME);
...
#ifdef CONFIG_PM
static int lsm303c_acc_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct lsm303c_acc_status *stat = i2c_get_clientdata(client);

	dev_dbg(dev, "%s: resume\n", LSM303C_ACC_DEV_NAME);
	lsm303c_acc_set_pinctrl_state(dev,	stat->pdata->pins_default);
...
static int lsm303c_acc_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct lsm303c_acc_status *stat = i2c_get_clientdata(client);

	dev_dbg(dev, "%s: suspend\n", LSM303C_ACC_DEV_NAME);
	lsm303c_acc_set_pinctrl_state(dev, stat->pdata->pins_sleep);
...
static const struct dev_pm_ops lsm303c_acc_pm = {
	SET_SYSTEM_SLEEP_PM_OPS(lsm303c_acc_suspend, lsm303c_acc_resume)
};
