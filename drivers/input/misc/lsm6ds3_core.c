...
#if defined (CONFIG_LSM6DS3_PINCTRL)
struct lsm6ds3_pinctrl_info{
...
static struct lsm6ds3_pinctrl_info *plsm6ds3_pinctrl = NULL;

#ifdef CONFIG_OF
static int acc_power_on(struct device *dev)
{
	struct device_pm_platdata *pm_platdata = plsm6ds3_pinctrl->pm_platdata;
...
static struct lsm6ds3_pinctrl_info *lsm6ds3_acc_of_get_platdata(
		struct device *dev)
{
	struct device_node *np = dev->of_node;
	struct lsm6ds3_pinctrl_info *acc_pdata;
//	u32 out_values[3];
	int ret = 0;
...
static inline int lsm6ds3_acc_set_pinctrl_state(struct device *dev,
		struct pinctrl_state *state)
{
	...
	if(NULL == plsm6ds3_pinctrl) {
		dev_err(dev,
			"plsm6ds3_pinctrl is NULL\n");
		return -EINVAL;
	}

	if (!IS_ERR_OR_NULL(state)) {
		ret = pinctrl_select_state(plsm6ds3_pinctrl->pinctrl, state);
		if (ret)
															...
															const char *description)
{
	...
	DTRACE("description=%s name=%s \n",description, lsm6ds3_sensor_name[sdata->sindex].name);
	...
	sdata->input_dev->name = lsm6ds3_sensor_name[sdata->sindex].name;
	//sdata->input_dev->name = description;
	input_set_drvdata(sdata->input_dev, sdata);

#if defined (CONFIG_INPUT_EVENT_REL)
	DTRACE("EVENT  REL\n");
	if(sdata->sindex == LSM6DS3_GYRO)
	{
		__set_bit(/*INPUT_EVENT_TYPE*/EV_REL, sdata->input_dev->evbit );
		//__set_bit(INPUT_EVENT_TIME_MSB, sdata->input_dev->mscbit);
		//__set_bit(INPUT_EVENT_TIME_LSB, sdata->input_dev->mscbit);
		__set_bit(/*INPUT_EVENT_X*/REL_RX, /*sdata->input_dev->mscbit*/sdata->input_dev->relbit);


		__set_bit(/*INPUT_EVENT_Y*/REL_RY, /*sdata->input_dev->mscbit*/sdata->input_dev->relbit);
		__set_bit(/*INPUT_EVENT_Z*/REL_RZ, /*sdata->input_dev->mscbit*/sdata->input_dev->relbit);

	}
	else if (sdata->sindex == LSM6DS3_ACCEL)
	{
		...
		/*	next is used for interruptB sources data if the case */
		input_set_abs_params(sdata->input_dev, ABS_WHEEL, INT_MIN,
									INT_MAX, 0, 0);
	}
	else
	{
													...
													s32 *xyz, s64 timestamp)
{
	...
	{
		input_event(input, /*INPUT_EVENT_TYPE*/EV_REL, REL_RX, xyz[0]);
		input_event(input, /*INPUT_EVENT_TYPE*/EV_REL, REL_RY, xyz[1]);
		input_event(input, /*INPUT_EVENT_TYPE*/EV_REL, REL_RZ, xyz[2]);
	}
#else
													...
													s32 data, s64 timestamp)
{
	...
	{
		input_event(input, EV_REL, REL_RX, data);
	}
#else
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_X, data);
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_MSB, timestamp >> 32);
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_LSB,														timestamp & 0xffffffff);
#endif
	input_sync(input);
}
...
enum hrtimer_restart lsm6ds3_poll_function_read(struct hrtimer *timer)
{
	...
	DTRACE("line=%d sdata->sindex=%d \n", __LINE__, sdata->sindex);
...
static void poll_function_work(struct work_struct *input_work)
{
	...
	int xyz[3] = { 0 };
	//u8 data[6];
	u8 data[LSM6DS3_OUT_XYZ_SIZE]={0};
	int err;
	...
	if (err < 0)
		...
		//DTRACE("line=%d xyz[0]=%d xyz[1]=%d xyz[2]=%d \n", __LINE__, xyz[0],xyz[1], xyz[2]);
		
		xyz[0] *= sdata->c_gain;
		xyz[1] *= sdata->c_gain;
		xyz[2] *= sdata->c_gain;

		//DTRACE("line=%d xyz[0]=%d xyz[1]=%d xyz[2]=%d after c_gain\n", __LINE__, xyz[0],xyz[1], xyz[2]);
...
static int lsm6ds3_enable_sensors(struct lsm6ds3_sensor_data *sdata)
{
	...
	case LSM6DS3_GYRO:
		...
		DTRACE("line=%d err=%d sdata->sindex=%d\n", __LINE__, err, sdata->sindex);
		if (err < 0)
			return err;
...
static int lsm6ds3_disable_sensors(struct lsm6ds3_sensor_data *sdata)
{
	...
	case LSM6DS3_ACCEL:
		...
		DTRACE("line=%d LSM6DS3_ACCEL err=%d\n", __LINE__, err);
		if (err < 0)
			return err;
	...
	case LSM6DS3_GYRO:
		err = lsm6ds3_write_data_with_mask(sdata->cdata,
									...
									LSM6DS3_ODR_POWER_OFF_VAL, true);
		DTRACE("line=%d LSM6DS3_GYRO err=%d\n", __LINE__, err);
		if (err < 0)
			return err;
...
static int gsensor_lsm6ds3_open(struct inode *inode, struct file *file)
{
    
    return 0;
}

/*ADC<C9>υτ<C7><FD><B6><AF><B5>?????<AF><CA><FD>*/
static int gsensor_lsm6ds3_release(struct inode *inode, struct file *filp)
{
    return 0;
}


 struct lsm6ds3_sensor_data *g_sdata = NULL;
 struct lsm6ds3_sensor_data *acc_sdata = NULL;
                                                 ...
                                                 unsigned int cmd, unsigned long arg)
{
   // int result = 0;
    ...
    DTRACE("cmd=%x GSENSOR_IOCTL_START=%x GSENSOR_IOCTL_CLOSE=%x \n", cmd, GSENSOR_IOCTL_START, GSENSOR_IOCTL_CLOSE);
    switch (cmd) {
        case GSENSOR_IOCTL_START:
			  if(acc_sdata != NULL)
			  	{
			  		DTRACE("acc_sdata is fine acc_sdata->sindex=%d LSM6DS3_GYRO=%d\n", acc_sdata->sindex, LSM6DS3_ACCEL);
              		lsm6ds3_enable_sensors(acc_sdata);
			  	}
               break;
         ...
         case GSENSOR_IOCTL_CLOSE:
		 	...
		 	{
		 	   DTRACE("acc_sdata is fine acc_sdata->sindex=%d LSM6DS3_GYRO=%d\n", acc_sdata->sindex, LSM6DS3_ACCEL);
              lsm6ds3_disable_sensors(acc_sdata);
		 	}
               break;
		 default:
                DTRACE("Unknow cmd\n");
...
/*<D7>?<FB><C9>υτ<B5><C4><CF><E0><B9>?<D9><D7><F7>?<CF><D6>*/
static struct file_operations gsensor_lsm6ds3_fops = 
{
    .owner    = THIS_MODULE,
    .open     = gsensor_lsm6ds3_open,
    .unlocked_ioctl     = gsensor_dev_ioctl,    
    .release  = gsensor_lsm6ds3_release,
};


static struct miscdevice gsensor_lsm6ds3_miscdev = 
{
    .minor   = MISC_DYNAMIC_MINOR, /*<B4><CE><C9>υτ<BA>?<AC><B6><A8><D2><E5><D4><DA>miscdevice.h<D6>§µ<AC>?255*/
    .name    = "mma8452_daemon",        /*<C9>υτ<C3><FB><B3><C6>*/
    .fops    = &gsensor_lsm6ds3_fops,          /*<B6><D4>ADC<C9>υτ<CE>?<FE><B2><D9><D7><F7>*/
};

 
static int gyro_lsm6ds3_open(struct inode *inode, struct file *file)
{
    
    return 0;
}

/*ADC<C9><F5><F4><C7><FD><B6><AF><B5>?????<AF><CA><FD>*/
static int gyro_lsm6ds3_release(struct inode *inode, struct file *filp)
{
    return 0;
}
...
/* ioctl - I/O control */
static long gyro_lsm6ds3_ioctl(struct file *file,
                                                 unsigned int cmd, unsigned long arg)
{
   					//printk("cdy == func = %s ,line = %d \n",__FUNCTION__,__LINE__);
	int result = 0;

	DTRACE("cmd=%x GSENSOR_IOCTL_START=%x GSENSOR_IOCTL_CLOSE=%x L3G4200D_IOCTL_SET_ENABLE=%x L3G4200D_IOCTL_GET_ENABLE=%x\n", cmd, GSENSOR_IOCTL_START, GSENSOR_IOCTL_CLOSE, L3G4200D_IOCTL_SET_ENABLE, L3G4200D_IOCTL_GET_ENABLE);
	DTRACE("L3G4200D_IOCTL_SET_DELAY=%x L3G4200D_IOCTL_GET_DELAY=%x\n", L3G4200D_IOCTL_SET_DELAY, L3G4200D_IOCTL_GET_DELAY);
 	if(g_sdata != NULL)
 	{
 		DTRACE("g_sdata is fine g_sdata->sindex=%d LSM6DS3_GYRO=%d\n", g_sdata->sindex, LSM6DS3_GYRO);
	    switch (cmd) {
			 case L3G4200D_IOCTL_SET_ENABLE:
			   if(*((int*)arg) == 1)
				{
					DTRACE("Enable call lsm6ds3_enable_sensors \n");
		         	result = lsm6ds3_enable_sensors(g_sdata);
					DTRACE("lsm6ds3_enable_sensors gyr result=%d \n", result);
					
				}else if(*((int*)arg) == 0)
				{
					DTRACE("Disable call lsm6ds3_disable_sensors \n");
					result =lsm6ds3_disable_sensors(g_sdata);
					DTRACE("lsm6ds3_disable_sensors gyr result=%d \n", result);

				}else
				{
					DTRACE("Notice, don't know arg value \n");
					result =0;
				}
		         break;
...
/*<D7>?<FB><C9><F5><F4><B5><C4><CF><E0><B9>?<D9><D7><F7>?<CF><D6>*/
static struct file_operations gyro_lsm6ds3_fops = 
{
    .owner    = THIS_MODULE,
    .open     = gyro_lsm6ds3_open,
    .unlocked_ioctl     = gyro_lsm6ds3_ioctl,    
    .release  = gyro_lsm6ds3_release,
};


static struct miscdevice gyro_lsm6ds3_miscdev = 
{
    .minor   = MISC_DYNAMIC_MINOR, /*<B4><CE><C9><F5><F4><BA>?<AC><B6><A8><D2><E5><D4><DA>miscdevice.h<D6><A7><B5><AC>?255*/
    .name    = "gyrosensor",        /*<C9><F5><F4><C3><FB><B3><C6>*/
    .fops    = &gyro_lsm6ds3_fops,          /*<B6><D4>ADC<C9><F5><F4><CE>?<FE><B2><D9><D7><F7>*/
};
...
int lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq, u16 bustype)
{
	...
	if (err < 0) {
		dev_err(cdata->dev, "failed to read Who-Am-I register. wai =0x%x .err = %d\n", wai,err);
		return err;
	}
	...
	dev_err(cdata->dev, "====== Who-Am-I register. wai =0x%x irq=%d.\n", wai, irq);
	
#if defined (CONFIG_LSM6DS3_PINCTRL)
	#ifdef CONFIG_OF
	plsm6ds3_pinctrl =
		lsm6ds3_acc_of_get_platdata(cdata->dev);
	if (IS_ERR(plsm6ds3_pinctrl)) {
		dev_err(cdata->dev, "lsm6ds3_acc_of_get_platdata failed.\n");
		err = PTR_ERR(plsm6ds3_pinctrl);
		return err;
	}
	#endif

	/*pcl*/
	lsm6ds3_acc_set_pinctrl_state(cdata->dev,
			plsm6ds3_pinctrl->pins_default);
#endif
	...
	for (i = 0; i < LSM6DS3_SENSORS_NUMB; i++) {
		...
		{
			g_sdata = sdata;
			DTRACE("line=%d poll_interval=%d \n", __LINE__, sdata->poll_interval);
		}
		else if(i == LSM6DS3_ACCEL)
		{
			acc_sdata = sdata;
			DTRACE("line=%d poll_interval=%d \n", __LINE__, sdata->poll_interval);
		}
...
#if 1
	err = misc_register(&gsensor_lsm6ds3_miscdev);
	if (err) 
	{
			   /*<B4><ED><CE><F3><B4><A6><C0><ED>*/
		printk(KERN_ERR "cannot register miscdev on minor=%d (%d)\n", MISC_DYNAMIC_MINOR, err);
	}
#endif

	 err = misc_register(&gyro_lsm6ds3_miscdev);
	 if (err) 
	 {
		/*<B4><ED><CE><F3><B4><A6><C0><ED>*/
...
#if defined (CONFIG_LSM6DS3_PINCTRL)
	lsm6ds3_acc_set_pinctrl_state(cdata->dev,
				plsm6ds3_pinctrl->pins_inactive);
#endif
	
	misc_deregister(&gyro_lsm6ds3_miscdev);
	misc_deregister(&gsensor_lsm6ds3_miscdev);
