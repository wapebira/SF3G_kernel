...
static void PointId(void)
{
	...
	if(global_state.only)
	{
		do
		{
			...
			{
				if(pp[1][0].all!=0 && pp[1][0].key != point_now[distance.j].key)
				{
...
static void PointOrder(void)
{
	...
	for(i=0;i<POINT_MAX;i++)
	{
		if(pp[0][i].fill == 0)
			continue;
		if(pp[1][i].all==0 || pp[1][i].fill!=0 || filter_able==0 || filter_able==1)
		{
			pp[0][i].all = 0;
			pressure_now[i] = 0;
		}
	...
	for(i=0;i<POINT_MAX;i++)
	{
		...
		if(i == 0)
		{
			...
			if(point_edge.rate!=0 && point_edge.dis!=0)
			{
				...
				ps[0][i].x = point_edge.coor.x + (ps[0][i].x - point_edge.coor.x)*(0x1000 - point_edge.rate)/0x1000;
				ps[0][i].y = point_edge.coor.y + (ps[0][i].y - point_edge.coor.y)*(0x1000 - point_edge.rate)/0x1000;
			}
		}
		if(ps[1][i].all == 0)
		{
		...
		for(i=0;i<POINT_MAX;i++)
		{
			...
			if(dc[0] > 0)
			{
				if(ps[0][i].all == ps[1][i].all && temp != ps[0][i].all)
				{
					...
					ps[0][i].x = ps[1][i].x;
					ps[0][i].y = ps[1][i].y;
					if(point_decimal[i].x >  dc[0] && ps[1][i].x < 0xffff)
					{
						ps[0][i].x += 1;
						point_decimal[i].x = 0;
					}
					if(point_decimal[i].x  < -dc[0] && ps[1][i].x > 0)
					{
						ps[0][i].x -= 1;
						point_decimal[i].x = 0;
					}
					if(point_decimal[i].y >  dc[0] && ps[1][i].y < 0xffff)
					{
						ps[0][i].y += 1;
						point_decimal[i].y = 0;
					}
					if(point_decimal[i].y  < -dc[0] && ps[1][i].y > 0)
					{
						ps[0][i].y -= 1;
						point_decimal[i].y = 0;
					}
	...
	else if(len >= 2)
	{
		...
		for(i=0;i<POINT_MAX;i++)
		{
			...
			if(dn < save_dn[i])
			{
				dr = save_dr[i];
				save_dn[i] = dn;
				ps[0][i].x = (int)ps[1][i].x + (((int)pp[0][i].x - (int)ps[1][i].x) * dr) / 128;
				ps[0][i].y = (int)ps[1][i].y + (((int)pp[0][i].y - (int)ps[1][i].y) * dr) / 128;
				continue;
			}
			for(j=0;j<=len;j++)
			{
...
static void DoubleClick(void)
{
	...
	if(point_num>=2 || (point_num==1 && pp[0][0].all==0) || pp[0][0].key)
	{
...
static void ChangeXY(void)
{
	...
	for(i=0;i<gesture_num && i<GESTURE_BUF_SIZE;i++)
		gesture_buf[i].all = ((gesture_buf[i].all & 0xfff) << 16) + ((gesture_buf[i].all>>16) & 0xffff);
}
...
//	int screen_x_max=0,screen_y_max=0;
	x1=gesture_buf[0].x;//480
	y1=gesture_buf[0].y;//800
	...
	for(i=1;i<gesture_num;i++)
	{
		if(abs(gesture_buf[i].x-middle_x)<(int)sen_num_nokey*64/x_scale)//screen_y_max/8)//30
			flag3|=0x1;
		else
			flag3|=0x2;
		if(abs(gesture_buf[i].y-middle_y)<(int)drv_num_nokey*64/y_scale)//screen_x_max/8)//25
			flag3|=(0x1<<4);
		else
			flag3|=(0x2<<4);
