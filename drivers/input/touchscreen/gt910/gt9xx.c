...
static const char * const key_names[] = {"Key_Home", "Key_Back",
				"Key_Menu", "Key_Search"};
#endif
...
static ssize_t gt91xx_config_read_proc(struct file *,
	char __user *, size_t, loff_t *);
static ssize_t gt91xx_config_write_proc(struct file *,
...
Output:
    None.
*******************************************************/
static int gtp_ts_power_off(struct i2c_client *client)
{
	...
	if (gt9xx_pdata->pins_sleep) {
		gt9xx_set_pinctrl_state(&client->dev,
			gt9xx_pdata->pins_sleep);
	}
#endif
	msleep(58);
	if (!ret)
...
Output:
    None.
*******************************************************/
static int gtp_ts_power_on(struct i2c_client *client)
{
	struct gt9xx_ts_platform_data *gt9xx_pdata;
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	int ret = -1;
...
static ssize_t gtp_ts_show_enable(struct device *dev,
...
static ssize_t gtp_ts_store_enable(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (val) {
	case 0:
		gtp_ts_power_off(client);
		break;
	case 1:
		gtp_ts_power_on(client);
		break;
	default:
...
static struct attribute *gtp_ts_attributes[] = {
	...
	if (DOZE_ENABLED == doze_status) {
		...
		if (ret > 0) {
			if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
				(doze_buf[2] == 'c') ||
				(doze_buf[2] == 'd') || (doze_buf[2] == 'e') ||
				(doze_buf[2] == 'g') ||
				(doze_buf[2] == 'h') || (doze_buf[2] == 'm') ||
				(doze_buf[2] == 'o') ||
				(doze_buf[2] == 'q') || (doze_buf[2] == 's') ||
				(doze_buf[2] == 'v') ||
...
*********************************************************/
static enum hrtimer_restart goodix_ts_timer_handler(
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
...
#endif
	{
		...
		if (ret == SUCCESS) {
			GTP_DEBUG(
				"CFG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
				sensor_id + 1,
	...
	if (CHIP_TYPE_GT9F == ts->chip_type) {
		...
		if (1 == ts->is_950) {
			driver_num =
			config[GTP_REG_MATRIX_DRVNUM - GTP_REG_CONFIG_DATA + 2];
			sensor_num =
			config[GTP_REG_MATRIX_SENNUM - GTP_REG_CONFIG_DATA + 2];
			if (have_key)
				driver_num--;
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	/*     ret = -ENODEV; */
	/* } */
	/* here do not change gpio to int pin, otherwise gpio ops
	in gtp_reset_guitar function can not work */
	...
	case GTP_BAK_REF_SEND:
		...
		for (j = 0; j < ref_grps; ++j) {
			...
			if (1 != ref_sum) {
				...
				memset(&p_bak_ref[j * ref_seg_len],
					0, ref_seg_len);
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
	...
	if (FAIL == ret) {
		...
		GTP_INFO("main clock data used fix num, main clock freq: %d",
			p_main_clk[0]);
	...
	if (IS_ERR(clk_filp)) {
		...
		clk_filp->f_op->read(clk_filp, (char *)p_main_clk,
			6, &clk_filp->f_pos);
	...
	struct i2c_client *client)
{
	...
	/* gpio reset */
	gt9xx_pdata->reset_pin = of_get_named_gpio_flags(
		client->dev.of_node,
	...
	const struct i2c_device_id *id)
{
...
#ifdef CONFIG_OF
	gt9xx_pdata = client->dev.platform_data =
		gt9xx_ts_of_get_platdata(client);
	...
	if (gt9xx_pdata->pm_platdata) {
		err = device_state_pm_set_class(&client->dev,
			gt9xx_pdata->pm_platdata->pm_user_name);
		if (err) {
			dev_err(&client->dev,
	...
	err = sysfs_create_group(&client->dev.kobj, &gtp_ts_attr_group);
	if (err)
...
{
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	struct gt9xx_ts_platform_data *gt9xx_pdata = client->dev.platform_data;

	GTP_DEBUG_FUNC();

	/* gt9xx_ts_power_off(ts); */
	if (gt9xx_pdata->pins_inactive) {
		gt9xx_set_pinctrl_state(&client->dev,
			gt9xx_pdata->pins_inactive);
	}
