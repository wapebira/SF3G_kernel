...
#endif
static int touch_register_fb(struct goodix_ts_data *ts);
static void touch_unregister_fb(struct goodix_ts_data *ts);
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].flags = GOODIX_I2C_WRITE_FLAG;
    msgs[0].addr  = client->addr;
    msgs[0].len   = GTP_ADDR_LENGTH;
    msgs[0].buf   = &buf[0];
    //msgs[0].scl_rate = 300 * 1000;    // for Rockchip, etc.
    
    msgs[1].flags = GOODIX_I2C_READ_FLAG;
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    msg.flags = GOODIX_I2C_WRITE_FLAG;
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    //here before GTP_INT_PORT gpio ops, we must use pin_config_set change int function to gpio
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    GTP_INT_SET_DEACTIVE(GTP_INT_PORT);
    mdelay(5);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    //    ret = -ENODEV;
    //}
    //here do not change gpio to int pin, otherwise gpio ops in gtp_reset_guitar function can not work
    //or must check GTP_GPIO_AS_INT define is Correct(2014.4.20)
    //else
    //{
    //    GTP_GPIO_AS_INT(GTP_INT_PORT);  
...
#else
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, GTP_MAX_WIDTH, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, GTP_MAX_HEIGHT, 0, 0);
#endif
...
#define OF_GT9XX_SUPPLY  "i2c"
#define OF_GT9XX_SUPPLY_A  "i2ca"
#define OF_GT9XX_PIN_RESET "intel,ts-gpio-reset"
#define OF_GT9XX_PIN_IRQ   "intel,ts-gpio-irq"

static struct gt9xx_ts_platform_data *gt9xx_ts_of_get_platdata(
	struct i2c_client *client)
{
	struct device_node *np = client->dev.of_node;
	struct gt9xx_ts_platform_data *gt9xx_pdata;
	struct regulator *pow_reg;
	int ret;

	gt9xx_pdata = devm_kzalloc(&client->dev,
							  sizeof(*gt9xx_pdata), GFP_KERNEL);
	if(!gt9xx_pdata)
		return ERR_PTR(-ENOMEM);

	/* regulator */
	pow_reg = regulator_get(&client->dev, OF_GT9XX_SUPPLY);
	gt9xx_pdata->power = pow_reg;
	if(IS_ERR(pow_reg))
	{
		dev_warn(&client->dev, "%s can't get %s-supply handle\n",
			 np->name, OF_GT9XX_SUPPLY);
		gt9xx_pdata->power = NULL;
	}

	pow_reg = regulator_get(&client->dev, OF_GT9XX_SUPPLY_A);
	gt9xx_pdata->power2 = pow_reg;
	if(IS_ERR(pow_reg))
	{
		dev_warn(&client->dev, "%s can't get %s-supply handle\n",
			 np->name, OF_GT9XX_SUPPLY_A);
		gt9xx_pdata->power2 = NULL;
	}

	/* pinctrl */
	gt9xx_pdata->pinctrl = devm_pinctrl_get(&client->dev);
	if(IS_ERR(gt9xx_pdata->pinctrl))
	{
		ret = PTR_ERR(gt9xx_pdata->pinctrl);
		goto out;
	}

	gt9xx_pdata->pins_default = pinctrl_lookup_state(
								gt9xx_pdata->pinctrl, PINCTRL_STATE_DEFAULT);
	if(IS_ERR(gt9xx_pdata->pins_default))
		dev_err(&client->dev, "could not get default pinstate\n");

	gt9xx_pdata->pins_sleep = pinctrl_lookup_state(
								gt9xx_pdata->pinctrl, PINCTRL_STATE_SLEEP);
	if(IS_ERR(gt9xx_pdata->pins_sleep))
    dev_err(&client->dev, "could not get sleep pinstate\n");

	gt9xx_pdata->pins_inactive = pinctrl_lookup_state(
                                gt9xx_pdata->pinctrl, "inactive");
	if(IS_ERR(gt9xx_pdata->pins_inactive))
		dev_err(&client->dev, "could not get inactive pinstate\n");

	/* gpio reset */
	gt9xx_pdata->reset_pin = of_get_named_gpio_flags(client->dev.of_node,
                            OF_GT9XX_PIN_RESET, 0, NULL);
	if(gt9xx_pdata->reset_pin <= 0)
	{
		dev_err(&client->dev,
	...
	/* gpio irq */
	gt9xx_pdata->irq_pin = of_get_named_gpio_flags(client->dev.of_node,
                            OF_GT9XX_PIN_IRQ, 0, NULL);
	if(gt9xx_pdata->irq_pin <= 0)
	{
		dev_err(&client->dev,
            "error getting gpio for %s\n", OF_GT9XX_PIN_IRQ);
		ret = -EINVAL;
	...
	if(of_property_read_bool(np, "intel,polling-mode"))
		gt9xx_pdata->polling_mode = true;

	/* pm */
	gt9xx_pdata->pm_platdata = of_device_state_pm_setup(np);
	if(IS_ERR(gt9xx_pdata->pm_platdata))
	{
		dev_warn(&client->dev, "Error during device state pm init\n");
		ret = PTR_ERR(gt9xx_pdata->pm_platdata);
		goto out;
	}
...
int gt9xx_set_pinctrl_state(struct device *dev,
    struct pinctrl_state *state)
{
	struct gt9xx_ts_platform_data *pdata = dev_get_platdata(dev);
	int ret = 0;
...
unsigned int* gt9xx_remap_vadd;   


int gt9xx_remap_ioset(int pin, int value)
{     
    iowrite32(value, gt9xx_remap_vadd + pin);
    return 0;
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gt9xx_remap_vadd = (unsigned int*)ioremap_nocache(0xE4600200, 0x1000);

    i2c_connect_client = client;

#ifdef CONFIG_OF
    gt9xx_pdata = client->dev.platform_data = gt9xx_ts_of_get_platdata(client);
    if(IS_ERR(gt9xx_pdata))
    {
        err = PTR_ERR(gt9xx_pdata);
        return err;
    }
#else
    ...
    if (!ts->pdata) {
        GTP_ERROR("No Platform data\n");
	return -1;
    }

	if (ts->pdata->pm_platdata) {
		err = device_state_pm_set_class(&client->dev,
			ts->pdata->pm_platdata->pm_user_name);
		if (err) {
			dev_err(&client->dev,
		...
		err = device_state_pm_set_state_by_name(&client->dev,
				ts->pdata->pm_platdata->pm_state_D0_name);
		if (err)
			return -1;
	}
...
static void goodix_ts_late_resume(struct early_suspend *h)
{
    ...
    ret = gt9xx_ts_power_on(ts);
    if (ret < 0) {
        GTP_ERROR("resume failed: %d\n", ret);
    }
...
#else
static int goodix_ts_suspend(struct i2c_client *client, pm_message_t state)
{
    s8 ret = -1;    
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
    ...
    //gt9xx_ts_power_off(ts);
	if (ts->pdata->pins_sleep)
		pinctrl_select_state(ts->pdata->pinctrl, ts->pdata->pins_sleep);
#if 0 //do not power off when suspend, or it will need reload firmware
	if (ts->pdata->pm_platdata &&
			ts->pdata->pm_platdata->pm_state_D3_name) {
...
static int goodix_ts_resume(struct i2c_client *client)
{
   ...
   //}
	...
	if (ts->pdata->pins_default)
		pinctrl_select_state(ts->pdata->pinctrl, ts->pdata->pins_default);
#if 0 // no power off no power on
	if (ts->pdata->pm_platdata &&
			ts->pdata->pm_platdata->pm_state_D0_name)
		ret = device_state_pm_set_state_by_name(&client->dev,
			ts->pdata->pm_platdata->pm_state_D0_name);
											...
											unsigned long event, void *data)
{
	...
	if (event != FB_EVENT_BLANK && event != FB_EVENT_CONBLANK)
		...
		ts->pdata->fb_blank = *(int *)evdata->data;
		if ((ts->pdata->fb_blank == FB_BLANK_UNBLANK) &&
				(ts->gtp_is_suspend == 1)) {
				goodix_ts_resume(i2c_connect_client);
				ts->gtp_is_suspend = 0;
			} else if (ts->pdata->fb_blank == FB_BLANK_POWERDOWN) {
				goodix_ts_suspend(i2c_connect_client, PMSG_SUSPEND);
				ts->gtp_is_suspend = 1;
			}
	return 0;
}
...
static int touch_register_fb(struct goodix_ts_data *ts)
{
	memset(&ts->pdata->fb_notif, 0, sizeof(&ts->pdata->fb_notif));
	ts->pdata->fb_notif.notifier_call = touch_fb_notifier_callback;

	return fb_register_client(&ts->pdata->fb_notif);
}

static void touch_unregister_fb(struct goodix_ts_data *ts)
{
	fb_unregister_client(&ts->pdata->fb_notif);
}
