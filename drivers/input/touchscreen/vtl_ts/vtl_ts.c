...
static void vtl_ts_report_xy_coord(struct ts_info *ts)
{
	...
	for (id = 0; id < touch_point_number; id++) {
		if ((xy_data->point[id].xhi != 0xFF) &&
			...
			if (ts->config_info->xy_swap_enable == 1){
		...
		if (ts->config_info->x_reverse_enable == 1){
			x = ts->config_info->screen_max_x - x;
		}
		if (ts->config_info->y_reverse_enable == 1){
...
static int vtltouch_fb_notifier_callback(struct notifier_block *self,
				unsigned long event, void *data)
{
	...
	DEBUG();
	if (ts->config_info->ctp_used) {
		ts->config_info->fb_blank = *(int *)evdata->data;
		if ((ts->config_info->fb_blank == FB_BLANK_UNBLANK)&&(ts->config_info->is_suspend == 1))
			{
				vtl_ts_resume(ts->driver->client);
				ts->config_info->is_suspend = 0;
			}
			else if(ts->config_info->fb_blank == FB_BLANK_POWERDOWN)
			{
				vtl_ts_suspend(ts->driver->client, PMSG_SUSPEND);
				ts->config_info->is_suspend = 1;
			}
	}
	return 0;
}
static int vtltouch_register_fb(struct ts_info *ts)
{
	memset(&ts->config_info->fb_notif, 0, sizeof(&ts->config_info->fb_notif));
	ts->config_info->fb_notif.notifier_call = vtltouch_fb_notifier_callback;

	return fb_register_client(&ts->config_info->fb_notif);
}

static void vtltouch_unregister_fb(struct ts_info *ts)
{
	fb_unregister_client(&ts->config_info->fb_notif);
}
	...
	struct i2c_client *client)
{
	...
	/* config */
	ret = of_property_read_u32(np,"xy-swap-enable",&vtl_pdata->xy_swap_enable);
	if (ret != 0) {
		pr_debug("get xy-swap-enable error\n");
		ret = -EINVAL;
		goto out;
	}
	ret = of_property_read_u32(np,"x-reverse-enable",&vtl_pdata->x_reverse_enable);
	if (ret != 0) {
		pr_debug("get x-reverse-enable error\n");
		ret = -EINVAL;
		goto out;
	}
	ret = of_property_read_u32(np,"y-reverse-enable",&vtl_pdata->y_reverse_enable);
	if (ret != 0) {
		pr_debug("get y-reverse-enable error\n");
		ret = -EINVAL;
		goto out;
	}
	ret = of_property_read_u32(np,"chip-diff",&vtl_pdata->chip_diff);
	if (ret != 0) {
		pr_debug("get chip-diff error\n");
		ret = -EINVAL;
		goto out;
	}
	return vtl_pdata;
