...
#define 	CHIP_DIFF_CT362		1
#define 	CHIP_DIFF_CT363S	2
...
static struct ts_info *ts_object;
static struct chip_cmd (*chip);
...
static int chip_i2c_read(struct i2c_client *client,
	__u16 addr, __u8 *buf, __u16 len)
{
	struct i2c_msg msgs;
	int ret;
	...
	if (ret != 1) {
		pr_debug("___%s:i2c read error___\n", __func__);
		return -1;
	}
	return 0;
}

static int chip_i2c_write(struct i2c_client *client,
	__u16 addr, __u8 *buf, __u16 len)
{
	struct i2c_msg msgs;
	int ret;
	...
	ret = i2c_transfer(client->adapter, &msgs, 1);
	if (ret != 1) {
		pr_debug("___%s:i2c write error___\n", __func__);
		return -1;
	}
	return 0;
}
...
static int chip_ram_write_1byte(unsigned short addr, unsigned char data)
{
	...
	buf[1] = addr >> 8;
	buf[2] = addr & 0x00ff;
	pr_debug("addr = %x,buf[0] = %x,buf[1] = %x,buf[2] = %x,data = %x\n",
		addr, buf[0], buf[1], buf[2], data);
	ret = chip_i2c_write(client, client->addr, buf, 3);
	pr_debug("first write end\n");
	if (ret)
		return ret;
	udelay(10);
	buf[0] = 0x00;
	...
	pr_debug("sec write end\n");
	udelay(10);
	return ret;
}

static int chip_ram_read(unsigned short addr, unsigned char *rx_buf,
	unsigned short len)
{
	struct i2c_client *client = ts_object->driver->client;
	unsigned char buf[3];
	...
	buf[1] = addr >> 8;
	buf[2] = addr & 0x00ff;
	pr_debug("addr = %x,buf[0] = %x,buf[1] = %x,buf[2] = %x\n",
...
int chip_enter_sleep_mode(void)
{
	...
	ret = chip_ram_write_1byte(chip[CHIP_SLEEP].addr,
		chip[CHIP_SLEEP].data);
	return ret;
}


int chip_function(enum cmd_index cmd_index,
	unsigned char *rx_buf, unsigned char len)
{
	...
	if (chip[cmd_index].data != RW_FLAG) { /*write*/
		ret = chip_ram_write_1byte(chip[cmd_index].addr,
			chip[cmd_index].data);
	} else {				  /*read*/
...
static int chip_read_bus_status(struct i2c_client *client,
...
static int chip_erase_flash(struct i2c_client *client)
{
	...
	DEBUG();
	/*ct36x mass erase*/
	if (chip[FLASH_MASS_ERASE_CMD].addr == 0x33) {
		ret = chip_read_bus_status(client, buf);
		if (buf[0] != 0xaa) {
			pr_debug("___i2c bus busy,bus_status = %d___\n",
				buf[0]);
			return -1;
		}
		buf[0] = 0x00;
		...
		if (ret) {
			pr_debug("vtl chip flash erase fail\n");
			return ret;
		}
		pr_debug("mass erase\n");
		msleep(20);
	} else {
		 /*ct360/ct36x sector erase*/
		for (sec = 0; sec < chip[FLASH_SECTOR_NUM].addr; sec++) {
			ret = chip_read_bus_status(client, buf);
			if (buf[0] != 0xaa) {
				pr_debug("___i2c bus busy,bus_status = %x,sec = %d___\n",
					buf[0], sec);
				return -1;
			}
...
static int chip_set_code(unsigned int flash_addr, unsigned char *buf)
{
	...
	buf[5] = ~(buf[2]+buf[3]+buf[4]+buf[6]+buf[7]+
		buf[8]+buf[9]+buf[10]+buf[11]+buf[12]+buf[13]) + 1;
	return buf[5];
}
...
static int chip_get_bin_checksum(void)
{
	...
	DEBUG();
	flash_addr = 0x00;
	cod = chip[FLASH_SECTOR_NUM].addr *
		(chip[FLASH_SECTOR_SIZE].addr/CHIP_FLASH_SOURCE_SIZE);
...
int chip_get_fwchksum(struct i2c_client *client, int *fwchksum)
{
	...
	ret = chip_ram_write_1byte(chip[FW_CHECKSUM_CMD].addr,
		chip[FW_CHECKSUM_CMD].data);
	if (ret)
		return -1;
...
int chip_get_checksum(struct i2c_client *client,
	int *bin_checksum, int *fw_checksum)
{
	...
	*bin_checksum = chip_get_bin_checksum();
	...
	pr_debug("bin_checksum = 0x%x,fw_checksum = 0x%x\n",
...
int update(struct i2c_client *client)
{
	...
	pr_debug("___chip update start___\n");
	...
	ret = chip_read_bus_status(client, buf);
	if (buf[0] != 0xaa) {
		pr_debug("___i2c bus busy,bus_status = %x___\n", buf[0]);
		return -1;
	}
	ret = chip_erase_flash(client);
	if (ret) {
		pr_debug("___erase flash fail___\n");
		return -1;
	}
	ret = chip_write_flash(client);
	if (ret) {
		pr_debug("___write flash fail___\n");
		return -1;
	}
	vtl_ts_hw_reset();
	pr_debug("___chip update end___\n");
...
int chip_update(struct i2c_client *client)
{
	...
	while ((bin_checksum != fw_checksum) && (cnt--)) {
		...
		chip_get_fwchksum(client, &fw_checksum);
		pr_debug("bin_checksum = %x,fw_checksum = %x,cnt = %d\n",
...
int chip_get_chip_id(struct i2c_client *client, unsigned char *rx_buf)
{
	...
	ret = chip_read_bus_status(client, buf);
	if (buf[0] != 0xaa) {
		pr_debug("___i2c bus status = %x,ret = %d___\n", buf[0], ret);
		return -1;
	}
	mdelay(1);
...
static int chip_write_infoblk(struct i2c_client *client)
{
	...
	for (cod = 0; cod < 16; cod++) {
		...
		buf[5] = (~(buf[2]+buf[3]+buf[4]+buf[6]+buf[7]+buf[8]
			+buf[9]+buf[10]+buf[11]+buf[12]+buf[13]))+1;
...
int chip_init(void)
{
	...
	switch(ts_object->config_info->chip_diff){
	case CHIP_DIFF_CT362:
		binary_data = binary_data_ct362;
		break;
	case CHIP_DIFF_CT363S:
		binary_data = binary_data_ct363s;
		break;
	default:
		break;
	}
	...
	for (retry = 0; retry < 3; retry++) {
		...
		pr_debug("___chip ID = %d___cnt = %d\n", chip_id, retry);
		switch (chip_id) {
			/*chip: CT362, CT363, CT365, CT368, CT369*/
			case 1:	{
					chip = ct36x_cmd;
	...
	/*chip: CT362M, CT363M, CT365M, CT368M, CT369M*/
			case 6:	{
					chip = ct36x_cmd;
				} break;
	...
	if (chip_update(client) < 0) {
		pr_debug("___chip updata faile___\n");
		return -1;
	}
#endif
