...
static int  TOUCH_DATA_LENGTH = 0;
...
static ssize_t icn85xx_tool_write(struct file * filp, const char __user *buff, size_t len, loff_t *off)
{
    int ret = 0;
    int i = 0;
    unsigned short addr;
    ...
    else if(15 == cmd_head.wr) // write hostcomm multibyte
    {
        ...
        addr = (cmd_head.addr[1]<<8) | cmd_head.addr[0];
        ret = icn85xx_i2c_txdata(addr, &cmd_head.data[i], cmd_head.data_len);
        if (ret < 0) {
    ...
    else if(17 == cmd_head.wr)// write iic porgmode multibyte
    {
        ...
        ret = icn85xx_prog_i2c_txdata(prog_addr, &cmd_head.data[i], cmd_head.data_len);
        if (ret < 0) {
            proc_error("write hostcomm multibyte failed!\n");
            goto write_out;
        }
...
static ssize_t icn85xx_tool_read(struct file *filp, char __user *page, size_t count, loff_t *off)
{
    int i, j;
    int ret = 0;
    ...
    else if(16 == cmd_head.wr)  //read gesture data
    {
        ...
        memcpy(&page[0], &structGestureData, sizeof(structGestureData));
    ...
    else if(18 == cmd_head.wr) // read hostcomm multibyte
    {
        ...
        addr = (cmd_head.addr[1]<<8) | cmd_head.addr[0];
        ret = icn85xx_i2c_rxdata(addr, &page[0], cmd_head.data_len);
        if (ret < 0) {
    ...
    else if(20 == cmd_head.wr)// read iic porgmode multibyte
    {
        ...
        prog_addr = (cmd_head.flag<<16) | (cmd_head.addr[1]<<8) | cmd_head.addr[0];
        icn85xx_goto_progmode();
        ret = icn85xx_prog_i2c_rxdata(prog_addr, &page[0], cmd_head.data_len);
        if (ret < 0) {
...
static void init_proc_node(void)
{
    int i;
    ...
    if (i)
    {
        //TOUCH_DATA_LENGTH = i * DATA_LENGTH_UINT + GTP_ADDR_LENGTH;
        TOUCH_DATA_LENGTH = i * DATA_LENGTH_UINT;
...
//        icn85xx_ts_reset();
    ...
    while(retries < IIC_RETRY_NUM)
    {
				if(this_client->adapter->nr == 4) {
					struct i2c_split_msg_info msgs_info[] = {{3, 0}, {0 ,0}};

					ret = i2c_transfer_split(this_client->adapter, msgs, 2, msgs_info, 32);
    		}
				else
...
int icn85xx_prog_i2c_txdata(unsigned int addr, char *txdata, int length)
{
    ...
    {
    		if(this_client->adapter->nr == 4) {
					struct i2c_split_msg_info msgs_info[] = {{3, 0}};

					ret = i2c_transfer_split(this_client->adapter, msg, 1, msgs_info, 32);
    		}
				else
...
int icn85xx_i2c_rxdata(unsigned short addr, char *rxdata, int length)
{
    ...
    {
    		if(this_client->adapter->nr == 4) {
					struct i2c_split_msg_info msgs_info[] = {{2, 0}, {0, 0}};
...
int icn85xx_i2c_txdata(unsigned short addr, char *txdata, int length)
{
    ...
    {
        if(this_client->adapter->nr == 4) {
					struct i2c_split_msg_info msgs_info[] = {{2, 0}};
...
{
    struct icn85xx_ts_data *ts = container_of(tp_d, struct icn85xx_ts_data, tp);
	//printk("----%s---\n", __func__);
	if (ts->is_suspend == 0) {
    	icn85xx_ts_suspend(ts->client, PMSG_SUSPEND);
		ts->is_suspend = 1;
	}
    return 0;
}
...
static void icn85xx_update(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    if(icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH)
    {
			...
			if (icn85xx_ts->board_info->screen_max_x == 600)
				icn85xx_set_fw(sizeof(icn85xx_600x1024_fw), &icn85xx_600x1024_fw[0]);
			else
	...
	struct i2c_client *client)
{
	...
	/*max x, max y*/
	ret = of_property_read_u32(np, "intel,max-x", &pdata->screen_max_x);
	if (ret) {
		dev_err(&client->dev, "error getting screen_max_x\n");
		ret = -EINVAL;
		goto out;
	}
	ret = of_property_read_u32(np, "intel,max-y", &pdata->screen_max_y);
	if (ret) {
		dev_err(&client->dev, "error getting screen_max_y\n");
		ret = -EINVAL;
		goto out;
	}

	/*revert x, y*/
	ret = of_property_read_u32(np, "intel,revert-x", &pdata->revert_x_flag);
	if (ret) {
		dev_err(&client->dev, "error getting screen_max_x\n");
		ret = -EINVAL;
		goto out;
	}
	ret = of_property_read_u32(np, "intel,revert-y", &pdata->revert_y_flag);
	if (ret) {
		dev_err(&client->dev, "error getting screen_max_y\n");
		ret = -EINVAL;
		goto out;
...
static int icn85xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    icn85xx_trace("20141014  ====%s begin=====.\n", __func__);
	...
	if (err) {
        icn85xx_error("icn83xx reset gpio request failed.\n");
		return -1;
	}
   ...
   // icn85xx_create_sysfs(client);
	...
	icn85xx_ts->tp.tp_resume = rk_ts_early_resume;
	icn85xx_ts->tp.tp_suspend = rk_ts_early_suspend;
	tp_register_fb(&icn85xx_ts->tp);
