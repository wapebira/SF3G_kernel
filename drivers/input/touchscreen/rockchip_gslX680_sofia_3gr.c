...
//#include "gslx680.h"
#include "rockchip_gslX680_sofia_3gr.h"
...
#include "../sensors/psensor/ps_tmd2772.h"

static u8 gsl_psensor_data[16]={0};
...
static struct i2c_client *ps_client = NULL;

 int tp_ps_init(struct i2c_client *client);
...
int gslx680_set_pinctrl_state(struct gsl_ts *ts,
...
static int gslX680_init(struct gsl_ts *ts)
{
	...
	ts->irq = of_get_named_gpio_flags(np, "intel,ts-gpio-irq", 0, NULL);
	ts->rst = of_get_named_gpio_flags(np, "intel,ts-gpio-reset", 0, NULL);
		
	//msleep(20);
	#if 0  //#if defined (CONFIG_BOARD_ZM71C)||defined (CONFIG_BOARD_ZM72CP)||defined (CONFIG_BOARD_ZM726C)||defined (CONFIG_BOARD_ZM726CE)
	...
	ts->pins_default= pinctrl_lookup_state(
		ts->pinctrl, PINCTRL_STATE_DEFAULT);
	//if (IS_ERR(ts->pins_default))
	//	dev_err(&client->dev, "could not get default pinstate\n");

	ts->pins_sleep = pinctrl_lookup_state(
		ts->pinctrl, PINCTRL_STATE_SLEEP);
	//if (IS_ERR(ts->pins_sleep))
	//	dev_err(&client->dev, "could not get sleep pinstate\n");

	ts->pins_inactive = pinctrl_lookup_state(
		ts->pinctrl, "inactive");
...
{
        struct i2c_msg xfer_msg[1];
        struct i2c_split_msg_info msgs_info = {1, 0};
	int ret = 0;
        buf[0] = reg;
        ...
        if(client->adapter->nr == 4)
        {
                ret = i2c_transfer_split(client->adapter, xfer_msg, 1, &msgs_info, 32);
                return (ret == 1) ? 0 : -EFAULT;
        }
        else
        {
...
static int gsl_ts_write(struct i2c_client *client, u8 addr, u8 *pdata, int datalen)
{
	...
	struct i2c_adapter *adap = client->adapter;
        ...
        if(adap->nr == 4)
        {
                ret = i2c_transfer_split(adap, &msg, 1, &msgs_info, 32);;
        }
        else
        {
...
static int tp_ps_getdata(char *data)
{
	...
	gsl_ts_read(i2c_client, 0xac, read_buf, sizeof(read_buf));
	*data = !(read_buf[0]);
//	PS_DBG("read_buf[0]=%d\n\n",read_buf[0]);
...
int tp_ps_enable(struct i2c_client *client, int enable, int rate)
{
	printk("%s ;%d\n", __func__,enable);
...
/*	
		buf[0]=0x3;
		gsl_ts_write(i2c_client,0xf0,buf,4);
		gsl_ts_read(i2c_client,0,&gsl_psensor_data[0],4);
		
		//buf[0]=0x4;
		//gsl_ts_write(i2c_client,0xf0,buf,4);
		//gsl_ts_read(i2c_client,0,&gsl_psensor_data[4],4);
		...
		buf[1] = 0x5a;
		buf[0] = 0x5a;
		gsl_ts_write(i2c_client, 0, buf, 4);
		buf[3] = 0x00;
		buf[2] = 0x00;
		buf[1] = 0x00;
		buf[0] = 0x4;
...
*/
 /*        buf[3] = 0x00;
        ...
        buf[1] = 0x0;
        buf[0] = 0x2;
        gsl_ts_write(i2c_client, 0x00, buf, 4);*/
		ps_en = 1;
	}
	else
	{
...
/*	
		...
		buf[0] = 0x4;
		gsl_ts_write(i2c_client, 0xf0, buf, 4);
		buf[3] = 0; //gsl_psensor_data[7];
		buf[2] = 0; //gsl_psensor_data[6];
		buf[1] = 0; //gsl_psensor_data[5];
		buf[0] = 0; //gsl_psensor_data[4];
		gsl_ts_write(i2c_client, 0, buf, 4);
*/      /* buf[3] = 0x00;
          buf[2] = 0x00;
        buf[1] = 0x00;
...
{
	struct sensor_private_data *sensor = (struct sensor_private_data *) i2c_get_clientdata(ps_client);	
	...
	input_report_abs(sensor->input_dev, ABS_DISTANCE, dps_data);
	input_sync(sensor->input_dev);

	return dps_data;
}

 int tp_ps_report(struct i2c_client *client)
{
/*	PS_DBG("--->gsl_ps_report!\n");
	ps_client = client;
	tp_ps_report_dps(0);
*/	return 0;
}
EXPORT_SYMBOL(tp_ps_report);

struct sensor_operate tp_ps_ops = {
	.name							= "ps_tmd2772",
...
static struct sensor_operate *tp_ps_get_ops(void)
{
	return &tp_ps_ops;
}
 ...
 int tp_ps_init(struct i2c_client *client)
{
	struct sensor_operate *ops = tp_ps_get_ops();
	int result 	= 0;
	int type 		= ops->type;
	...
	PS_DBG("--->tp_ps_init!\n");
//	result = sensor_register_slave(type, NULL, NULL, tp_ps_get_ops);
	return result;
}
EXPORT_SYMBOL(tp_ps_init);
static int tp_ps_uninit(void)
{
	struct sensor_operate *ops = tp_ps_get_ops();
	int type = ops->type;
	
	sensor_unregister_slave(type, NULL, NULL, tp_ps_get_ops);
	ps_en = 0;
	return 0;
}
...
static void gslX680_ts_worker(struct work_struct *work)
{
    ...
    PS_DBG("-----touches: %d -report?=%d,suspend_entry_flag=%d----\n", touches,ps_en,suspend_entry_flag);		
...
//	print_info("----------------gsl_monitor_worker-----------------\n");	
   struct gsl_ts *ts = container_of(work, struct gsl_ts,gsl_monitor_work.work);
	if(i2c_lock_flag != 0)
	{
	i2c_lock_flag=1;
	}	
...
//#endif	
	...
	rc=  request_irq(client->irq, gsl_ts_irq, IRQF_TRIGGER_FALLING, client->name, ts);
	if (rc < 0) {
		printk( "gsl_probe: request irq failed\n");
		goto error_req_irq_fail;
	}

	pr_info("%s:client->irq=%d, ts->irq=%d\n",__func__, client->irq, ts->irq);
