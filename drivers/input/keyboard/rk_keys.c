...
static int keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ddata->nbuttons; i++) {
		...
		if (button->type == TYPE_GPIO) {
			...
			irqflag = button->active_low ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING;
			if (button->wakeup)
				irqflag |= IRQF_NO_SUSPEND;

			error = devm_request_irq(dev, irq, keys_isr, irqflag,
						 button->desc ?
						 button->desc : "keys",
						 button);
			if (error) {
