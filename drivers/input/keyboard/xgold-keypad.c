...
static struct input_dev *xgold_input_dev = NULL;
void xgold_send_power_key(int state)
{
    if (!xgold_input_dev)
            return;
    if(state)
    {
        input_report_key(xgold_input_dev, KEY_POWER, 1);
        input_sync(xgold_input_dev);
    }
    else
    {
        input_report_key(xgold_input_dev, KEY_POWER, 0);
        input_sync(xgold_input_dev);
    }
}
EXPORT_SYMBOL(xgold_send_power_key);

void xgold_send_wakeup_key(void)
{
    if (!xgold_input_dev)
        return;
    input_report_key(xgold_input_dev, KEY_WAKEUP, 1);
    input_sync(xgold_input_dev);
    input_report_key(xgold_input_dev, KEY_WAKEUP, 0);
    input_sync(xgold_input_dev);
}
EXPORT_SYMBOL(xgold_send_wakeup_key);
...
/*
	dev_err(&keypad->pdev->dev, "key_num1 %x, old_key_num1%x\n",
				key_num->key_num1, old_key_num->key_num1);
	dev_err(&keypad->pdev->dev, "key_num2 %x , old_key_num2 %x\n",
				key_num->key_num2, old_key_num->key_num2);
	dev_err(&keypad->pdev->dev, "special_key %x, special_key_old %x\n",
				*special_key_matrix, *special_key_old_matrix);
*/
	if (*special_key_matrix != *special_key_old_matrix) {
//		printk("special_key_matrix have detect\n");
...
static irqreturn_t xgold_keypad_irq_handler(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&keypad->suspend_lock, 3*HZ);
	reg_val = kpd_read32(keypad, KPD_MIS);

	//dev_err(&keypad->pdev->dev, "KPD_MIS %x\n", reg_val);
				 ...
				 struct xgold_kpd_device *keypad_data)
{
	...
	/*kp_in for keys*/
	error = of_property_read_u32(np,"keypad,kp_in0",(u32 *)&(keypad_data->keycodes[64])); 
	if(error!=0){
		dev_err(dev, "get keypad,kp_in0 error\n");
	}
	error = of_property_read_u32(np,"keypad,kp_in1",(u32 *)&(keypad_data->keycodes[65])); 
	if(error!=0){
		dev_err(dev, "get keypad,kp_in1 error\n"); 
	}
	error = of_property_read_u32(np,"keypad,kp_in2",(u32 *)&(keypad_data->keycodes[66]));
	if(error!=0){
		dev_err(dev, "get keypad,kp_in2 error\n");
	}
	error = of_property_read_u32(np,"keypad,kp_in3",(u32 *)&(keypad_data->keycodes[67])); 
	if(error!=0){
		dev_err(dev, "get keypad,kp_in3 error\n"); 
	}
	error = of_property_read_u32(np,"keypad,kp_in4",(u32 *)&(keypad_data->keycodes[68]));
	if(error!=0){
		dev_err(dev, "get keypad,kp_in4 error\n");
	}
	error = of_property_read_u32(np,"keypad,kp_in5",(u32 *)&(keypad_data->keycodes[69]));
	if(error!=0){
		dev_err(dev, "get keypad,kp_in5 error\n");
	}	
	error = of_property_read_u32(np,"keypad,kp_in6",(u32 *)&(keypad_data->keycodes[70]));
	if(error!=0){
		dev_err(dev, "get keypad,kp_in6 error\n");
	}	
	error = of_property_read_u32(np,"keypad,kp_in7",(u32 *)&(keypad_data->keycodes[71]));
	if(error!=0){
...
static int xgold_keypad_probe(struct platform_device *pdev)
{
	...
	printk("xgold_keypad_probe main irq = %d\n",irq);
	...
	printk("=============xgold_keypad_probe, here dump dts configs for kpd=======================\n");
	printk("num_row_gpios = %d, num_col_gpios = %d\n",keypad->pdata->key_cfg->num_row_gpios,keypad->pdata->key_cfg->num_col_gpios);
	...
	/* Set up the wake lock to prevent suspend when irq happening. */
	wake_lock_init(&keypad->suspend_lock,
				WAKE_LOCK_SUSPEND,
				"keypad_wake_lock");
...
static int xgold_keypad_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&keypad->suspend_lock);
