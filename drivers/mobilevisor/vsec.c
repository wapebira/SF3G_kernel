...
u32 vsec_init(void)
{
	...
	if (p_vsec_ctx->vsec_share == NULL)
		return -EINVAL;

	p_vsec_ctx->vsec_share->handshake = mv_gal_os_id();
	p_vsec_ctx->vsec_rpc_ctx[0].data = p_vsec_ctx->vsec_share->share_data;
	p_vsec_ctx->vsec_rpc_ctx[0].max_size =
		(p_vsec_ctx->share_mem_size - sizeof(struct vsec_share_ctx))/2;

	p_vsec_ctx->vsec_rpc_ctx[1].data =
		p_vsec_ctx->vsec_rpc_ctx[0].data + p_vsec_ctx->vsec_rpc_ctx[0].max_size;
