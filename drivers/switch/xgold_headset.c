...
static void headset_switch_work(struct work_struct *work)
{
	...
	int ret, volt, retries = 0, count = 0;
	enum xgold_headset_config val;
	int hook_enable = -1;
	...
	do {
		msleep(250);
		do {
			ret = iio_st_read_channel_raw(data->iio_client, &volt);
			count++;
		} while ((ret < 0) && (count < 5));
		if (ret < 0) {
			dev_err(data->sdev.dev, "Unable to read channel volt\n");
		...
		old_state = data->pdata->xgold_read_state(
								...
								data,
								volt);
		dev_dbg(data->sdev.dev, "old_state = %u volt = %d\n",
			(u32)old_state, volt);
		...
		state = data->pdata->xgold_read_state(
								...
								data,
								volt);
		dev_dbg(data->sdev.dev, "state = %u volt = %d\n",
			(u32)state, volt);
		retries++;
	} while ((state != old_state) && (retries < 5));
	if (retries >= 5) {
		dev_err(data->sdev.dev, "MIC line voltage is not stable. Cannot proceed with identification/n");
		wake_unlock(&data->acc_lock);
		enable_irq(data->irq_headset);
		return;
	}
