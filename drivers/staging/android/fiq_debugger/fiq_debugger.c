...
struct fiq_debugger_state {
	...
	bool console_enable;
	bool console_enable_while_debug;
	int current_cpu;
			...
			void *svc_sp)
{
	...
	if (!strcmp(cmd, "help") || !strcmp(cmd, "?")) {
		...
		state->console_enable_while_debug = true;
		show_regs((struct pt_regs *)regs);
		state->console_enable_while_debug = false;
...
static irqreturn_t fiq_debugger_uart_irq(int irq, void *dev)
{
	...
	if (state->pdata->irq_mask)
		state->pdata->irq_mask(state->pdev);
	...
	if (state->pdata->irq_unmask)
		state->pdata->irq_unmask(state->pdev);
				...
				const char *s, unsigned int count)
{
	...
	if (state->console_enable_while_debug) {
		while (count--) {
			if (*s == '\n')
				fiq_debugger_putc(state, '\r');
