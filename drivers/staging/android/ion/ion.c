...
#if defined(CONFIG_ION_ROCKCHIP_SNAPSHOT)
#include "rockchip/rockchip_ion_snapshot.h"
#endif
#endif
#include "ion.h"
#include "ion_priv.h"
...
static int ion_cma_heap_debug_show(struct seq_file *s, void *unused)
{
	...
	rows = cma->count/(SZ_1M >> PAGE_SHIFT);
	base = __pfn_to_phys(cma->base_pfn);

	seq_printf(s, "%s Heap bitmap:\n", heap->name);

	for (i = rows - 1; i >= 0; i--) {
		seq_printf(s, "%.4uM@0x%08x: %08lx %08lx %08lx %08lx\n"
			   " %08lx %08lx %08lx %08lx\n",
			   i+1, base+(i)*SZ_1M,
			   cma->bitmap[i*8 + 7],
			   cma->bitmap[i*8 + 6],
...
void ion_device_add_heap(struct ion_device *dev, struct ion_heap *heap)
{
	...
	if (ION_HEAP_TYPE_DMA == heap->type) {
		char *heap_bitmap_name = kasprintf(GFP_KERNEL,
						   "%s-bitmap",
						   heap->name);
...
#ifdef CONFIG_ION_ROCKCHIP_SNAPSHOT
static unsigned long ion_find_max_zero_area(unsigned long *map,
					    unsigned long size)
{
...
static int ion_snapshot_save(struct ion_device *idev, size_t len)
{
	...
	plist_for_each_entry(heap, &idev->heaps, node) {
		...
		if (ION_HEAP_TYPE_DMA == heap->type) {
			struct ion_cma_heap *cma_heap = container_of(
					heap,
					struct ion_cma_heap,
					heap);
			...
			seq_printf(&seqf, "\n");
			seq_printf(&seqf,
				 "Maximum allocation of pages: %ld\n",
				 ion_find_max_zero_area(
					 cma->bitmap,
					 cma->count));
			seq_printf(&seqf, "\n");
		}
