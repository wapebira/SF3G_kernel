		...
		struct device_node *node)
{
	...
	for (itype = 0;	itype < ARRAY_SIZE(ion_heap_type_name);	itype++) {
		if (!strcmp(ion_heap_type_name[itype].name, node->name)) {
			...
			/* Read the customized id from dtb */
			ret = of_property_read_u32(node, "id", &myheap->id);
			if (ret)
				myheap->id = ion_heap_type_name[itype].id;

			if (ION_HEAP_TYPE_SYSTEM == myheap->type) {
				pr_info("xg_ion: find heap: system-heap\n");
				return 1;
			}
			...
			case ION_HEAP_TYPE_DMA:
				if (cma_area) {
					...
					pr_err("define DMA heap w/o cma_area\n");
					continue;
				}
				break;
			...
			case ION_HEAP_TYPE_CARVEOUT:
				if (cma_area) {
					pr_err("define CARVEOUT heap with cma_area\n");
					continue;
				}
				if (!myheap->base) {
					pr_err("define CARVEOUT heap w/o base\n");
					continue;
				}
				break;
			...
			case ION_HEAP_TYPE_SECURE:
				if (!size) {
					pr_err("define SECURE heap w/o size\n");
					continue;
				}
				break;
