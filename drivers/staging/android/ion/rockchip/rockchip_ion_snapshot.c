...
static int __init rockchip_ion_snapshot_init(void)
{
	...
	log_buf = (char *)__get_free_pages(GFP_KERNEL,
		LOG_BUF_PAGE_ORDER);
	if (!log_buf) {
	...
	ion_snapshot_buf = last_ion_vmap(virt_to_phys(log_buf),
	...
	pr_info("0x%08x map to 0x%p and copy to 0x%p (version 0.1)\n",
		virt_to_phys(log_buf),
		ion_snapshot_buf, last_ion_buf);
...
module_init(rockchip_ion_snapshot_init);
