...
static void xgold_usif_drain_tx_fifo(struct uart_port *port)
{
	int tries = 0;
	...
	if (xgold_usif_is_clk_enabled(port)) {
		while (USIF_FIFO_STAT_TXFFS(
			...
			/* The time it takes to drain the TX FIFO depends on
			 * the fill level, the FIFO size and the baud rate.
			 * Even at a low baud rate this should not take longer
			 * than a few ms so waiting a total of 60ms should be
			 * sufficient.
			 */
			tries++;
			msleep(20);
			if (tries > 3)
				/* Remote side may not be ready to receive
				 * data. Break to prevent a deadlock.
				*/
				break;
		}
	}
...
static inline void xgold_usif_putchar(struct uart_port *port, int s)
{
	while (USIF_FIFO_STAT_TXFFS(ioread32(USIF_FIFO_STAT(port->membase)))) {
		barrier();
		/* Add delay here to prevent stall caused by too
		 * frequent polling of the status register.
		 * The shortest measured time we poll here is just above 10us.
		 * We don't want to delay here longer than necessary so
		 * a delay of 10us seems appropriate.
		 */
		udelay(10);
	}
