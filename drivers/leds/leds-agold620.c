...
#define SCU_LED_UP_CMP_100mv	0x00106
#define SCU_LED_UP_CMP_200mv	0x00116
...
static inline void agold620_led_on(struct device *dev)
{
	...
	if (freerun_mode) {
		...
		val = led->config.min_br * 100 + (led->led_brightness * 100 * (led->config.max_br - led->config.min_br) / 255);
		val = (TOTAL_CLK_20K_IN_26M * val) / 255 / 100;

		if (led->config.polarity) {
			led_write32(led, LED_K2_CONTROL, TOTAL_CLK_20K_IN_26M - val);
			led_write32(led, LED_K1_CONTROL, val);
		} else {
			if(val == TOTAL_CLK_20K_IN_26M)
				val -= 1;
			led_write32(led, LED_K2_CONTROL, val);
			led_write32(led, LED_K1_CONTROL, TOTAL_CLK_20K_IN_26M - val);
		}

	} else {
		int32_t val = (SCU_K2_VAL * 100);
		intensity = SCALING_INTENSITY(led->led_brightness);
...
static int32_t agold620_set_gpio(struct device *dev, bool on)
{
	...
	if (gpio_is_valid(led->gpio)) {
		if (on == true)
			gpio_direction_output(led->gpio, (led->gpio_flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1);
		else
			gpio_direction_output(led->gpio, (led->gpio_flags == OF_GPIO_ACTIVE_LOW) ? 1 : 0);
...
int32_t agold620_led_shutdown(struct platform_device *pdev)
{
		return xgold_led_remove(pdev);
}
