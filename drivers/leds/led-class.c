				...
				unsigned long event, void *data)
{
	...
	struct led_classdev *led_cdev = container_of(self,
					struct led_classdev, notifier);
	/* If we aren't interested in this event, skip it immediately ... */
	if (event != FB_EVENT_BLANK && event != FB_EVENT_CONBLANK)
		return 0;
	if (led_cdev->name != "lcd-backlight" && led_cdev->name != "keyboard-backlight")
		return 0;

		led_cdev->fb_blank = *(int *)evdata->data;
		if (led_cdev->fb_blank == FB_BLANK_UNBLANK) {
			/*resume*/
				/*printk("led_classdev_resume\n");*/
				led_classdev_resume(led_cdev);
		} else if (led_cdev->fb_blank == FB_BLANK_POWERDOWN) {
			/*suspend*/
				/*printk("led_classdev_suspend\n");*/
				led_classdev_suspend(led_cdev);
		}
...
int led_classdev_register(struct device *parent, struct led_classdev *led_cdev)
{
	...
	led_cdev->notifier.notifier_call = fb_notifier_callback;
	ret = fb_register_client(&led_cdev->notifier);
	if (ret)
		dev_err(led_cdev->dev, "unable to register backlight trigger\n");
#ifdef CONFIG_LEDS_TRIGGERS
