...
enum prop_trsm_type {
	PROP_INNER_CONNECT = 0,
	PROP_EXTER_CONNECT,
};

enum black_mode {
	DSP_BLACK_UNBLACK = 0,
	DSP_BLACK_NORMAL,	/* standby vop and disable transmitter */
	DSP_BLACK_TRSM,		/* only disable transmitter */
};
...
struct rockchip_vop_drv_ops {
	...
	int (*dsp_black)(struct rockchip_vop_driver *dev_drv, int enable);
};
...
#elif defined(CONFIG_ION_ROCKCHIP)
extern struct ion_client *rockchip_ion_client_create(const char *name);
#else
static struct ion_client *xgold_ion_client_create(const char *name)
{
	return NULL;
}

static struct ion_client *rockchip_ion_client_create(const char *name)
{
	return NULL;
}
#endif
