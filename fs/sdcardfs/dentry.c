...
static unsigned int __sdcardfs_striptail_len(unsigned int len, const char *name)
{
...
static unsigned int sdcardfs_striptail_len(const struct qstr *qstr)
{
	return __sdcardfs_striptail_len(qstr->len, qstr->name);
}
