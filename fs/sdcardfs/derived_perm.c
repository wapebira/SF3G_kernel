...
void get_derived_permission(struct dentry *parent, struct dentry *dentry)
{
	...
	case PERM_LEGACY_PRE_ROOT:
		...
		info->perm = PERM_ROOT;
		info->userid = simple_strtoul(dentry->d_name.name,
					      NULL, 10);
		break;
	case PERM_ROOT:
	...
	case PERM_ANDROID:
		if (!strcasecmp(dentry->d_name.name, "data")) {
			...
			/* User directories must only be accessible to
			 * system, protected by sdcard_all. Zygote will
			 * bind mount the appropriate user-specific path.
			 */
	...
	case PERM_ANDROID_MEDIA:
		...
		if (appid != 0)
			info->d_uid = multiuser_get_uid(parent_info->userid,
							appid);
...
inline void update_derived_permission(struct dentry *dentry)
{
	...
	if (!dentry || !dentry->d_inode) {
		pr_debug("sdcardfs: %s: invalid dentry\n", __func__);
		return;
	}
	/* FIXME:
	...
	if (di->orig_path.dentry) {
		if (!di->lower_path.dentry) {
			...
			if (!path_buf) {
				...
				obbpath_s = d_path(&di->lower_path,
						   path_buf, PATH_MAX);
...
int setup_obb_dentry(struct dentry *dentry, struct path *lower_path)
{
	...
	if (!err) {
		...
		pr_debug("sdcardfs: the sbi->obbpath is found\n");
