			   ...
			   umode_t mode, bool excl)
{
	...
	int has_rw = get_caller_has_rw_locked(sbi->pkgl_id,
					      sbi->options.derive);
	if (!check_caller_access_to_name(dir,
					 dentry->d_name.name,
					 sbi->options.derive,
					 1,
					 has_rw)) {
		pr_debug("%s: need to check the caller's gid in packages.list\n \
			 dentry: %s, task:%s\n",
...
#ifdef SDCARDFS_SUOOPRT_SYMLINK
...
{
	struct file *filp = filp_open(abs_path,
				      O_RDWR | O_CREAT |
				      O_EXCL | O_NOFOLLOW,
				      mode);
	if (IS_ERR(filp)) {
			  ...
			  umode_t mode)
{
	...
	if (!check_min_free_space(dentry, 0, 1)) {
		pr_debug("sdcardfs: No minimum free space.\n");
		err = -ENOSPC;
		goto out_revert;
	}
	...
	if (need_graft_path(dentry)) {
		...
		if (err) {
			/* if the sbi->obbpath is not available,
			* the lower_path won't be changed by setup_obb_dentry()
			* but the lower path is saved to its orig_path. this
			* dentry will be revalidated later. But now, the
			* lower_path should be NULL */
			sdcardfs_put_reset_lower_path(dentry);

			/* the newly created lower path which saved to its orig
			* path or the lower_path is the base obbpath.
	...
	/* When creating /Android/data and /Android/obb,
	* mark them as .nomedia */
	if (make_nomedia_in_obb ||
		...
		nomedia_dir_name = d_absolute_path(&lower_path,
						   page_buf, PAGE_SIZE);
...
static int sdcardfs_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	/* sdcardfs_get_real_lower(): in case of remove an user's
	* obb dentry the dentry on the original path should be deleted. */
...
#ifdef SDCARDFS_SUPPORT_MKNOD
static int sdcardfs_mknod(struct inode *dir,
			  struct dentry *dentry,
			  umode_t mode,
			  dev_t dev)
{
			   ...
			   struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (new_dir != old_dir) {
		sdcardfs_copy_inode_attr(old_dir,
		...
		if (new_parent) {
			if (old_dentry->d_inode) {
				get_derived_permission(new_parent,
						       old_dentry);
...
#ifdef SDCARDFS_SUPPORT_LINK
static int sdcardfs_readlink(struct dentry *dentry,
...
static void sdcardfs_put_link(struct dentry *dentry,
			      struct nameidata *nd,
			      void *cookie)
{
...
static int sdcardfs_getattr(struct vfsmount *mnt,
			    struct dentry *dentry,
			    struct kstat *stat)
{
	struct dentry *lower_dentry;
	...
	if (!check_caller_access_to_name(parent->d_inode,
					 dentry->d_name.name,
...
static int sdcardfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	if (!err) {
		...
		has_rw = get_caller_has_rw_locked(sbi->pkgl_id,
						  sbi->options.derive);
		parent = dget_parent(dentry);
		if (!check_caller_access_to_name(parent->d_inode,
						 dentry->d_name.name,
						 sbi->options.derive, 1,
						 has_rw)) {
			pr_debug("%s: check the caller's gid in packages.list\n\
				 dentry: %s, task:%s\n",
				 __func__, dentry->d_name.name, current->comm);
			err = -EACCES;
		}
