...
int get_caller_has_rw_locked(void *pkgl_id, derive_t derive)
{
	...
	kuid = current_fsuid();
	appid = multiuser_get_app_id(kuid.val);
...
appid_t get_appid(void *pkgl_id, const char *app_name)
{
	...
	mutex_lock(&pkgl_dat->hashtable_lock);
	hash_for_each_possible(pkgl_dat->package_to_appid, hash_cur,
			       hlist, hash) {
...
int check_caller_access_to_name(struct inode *parent_node, const char *name,
	...
	if (w_ok) {
		if (parent_node &&
		    (kuid.val == SDCARDFS_I(parent_node)->d_uid)) {
			return 1;
		}
		return has_rw;
	}
...
static int insert_str_to_int(struct packagelist_data *pkgl_dat,
			     void *key, int value) {
	struct hashtable_entry *hash_cur;
...
static int insert_int_to_null(struct packagelist_data *pkgl_dat,
			      void *key, int value)
{
	struct hashtable_entry *hash_cur;
	struct hashtable_entry *new_entry;

	hash_for_each_possible(pkgl_dat->appid_with_rw,	hash_cur,
			       hlist, (uintptr_t)key) {
		if (key == hash_cur->key) {
			hash_cur->value = value;
			return 0;
...
static void remove_all_hashentrys(struct packagelist_data *pkgl_dat)
{
	...
	hash_for_each_safe(pkgl_dat->package_to_appid, i, h_t,
			   hash_cur, hlist)
		remove_str_to_int(hash_cur);

	hash_for_each_safe(pkgl_dat->appid_with_rw, i, h_t,
			   hash_cur, hlist)
...
static int read_package_list(struct packagelist_data *pkgl_dat)
{
	...
	pr_debug("sdcardfs: read_package_list\n");
	...
	if (fd < 0) {
		pr_err("sdcardfs: failed to open package list\n");
		mutex_unlock(&pkgl_dat->hashtable_lock);
		return fd;
	}
...
static int packagelist_reader(void *thread_data)
{
	...
	if (nfd < 0) {
		pr_err("sdcardfs: inotify_init failed: %d\n", nfd);
		return nfd;
	}
	...
	while (!kthread_should_stop()) {
		...
		if (!active) {
			...
			if (res < 0) {
				...
				sleep and retry */
					pr_err("sdcardfs: missing packages.list;\
					       retrying\n");
					ssleep(2);
					pr_err("sdcardfs: missing packages.list\
					       end; retrying\n");
					...
					pr_err("sdcardfs: inotify_add_watch\
					       failed: %d\n", res);
			...
			if (res) {
				pr_err("sdcardfs: read_package_list failed: %d\n",
				       res);
				goto interruptable_sleep;
			}
			active = true;
		...
		if (res < (int)sizeof(*event)) {
			if (res == -EINTR)
				continue;
			pr_err("sdcardfs: failed to read inotify event: %d\n",
			       res);
			goto interruptable_sleep;
		}
		...
		while (res >= (int)sizeof(*event)) {
			...
			pr_debug("sdcardfs: inotify event: %08x\n",
				 event->mask);
			if ((event->mask & IN_IGNORED) == IN_IGNORED) {
				/* Previously watched file was deleted,
				* probably due to move that swapped in new data;
				* re-arm the watch and read. */
				active = false;
			}
...
void *packagelist_create(gid_t write_gid)
{
	...
	packagelist_thread = kthread_run(packagelist_reader,
					 (void *)pkgl_dat, "pkgld");
	if (IS_ERR(packagelist_thread)) {
		pr_err("sdcardfs: creating kthread failed\n");
		kfree(pkgl_dat);
		return packagelist_thread;
	}
	pkgl_dat->thread_id = packagelist_thread;

	pr_debug("sdcardfs: created packagelist pkgld/%d\n",
...
void packagelist_destroy(void *pkgl_id)
{
	...
	remove_all_hashentrys(pkgl_dat);
	pr_debug("sdcardfs: destroyed packagelist pkgld/%d\n", (int)pkgl_pid);
	kfree(pkgl_dat);
}
