...
invalid_option:
			if (!silent) {
				pr_debug("Unrecognized mount option \"%s\"\
					 or missing value", p);
			}
			return -EINVAL;
		}
			       ...
			       void *raw_data, int silent)
{
	...
	if (!SDCARDFS_SB(sb)) {
		pr_err("sdcardfs: read_super: out of memory\n");
		err = -ENOMEM;
		goto out_free;
	}
	...
	if (err) {
		pr_err("sdcardfs: invalid options or out of memory\n");
		goto out_freesbi;
	}
	...
	if (!err) {
		...
		case DERIVE_NONE:
			...
			break;
		case DERIVE_LEGACY:
			/* Legacy behavior used to support internal
			* multiuser layout which places user_id at the
			* top directory level, with the actual roots
			* just below that. Shared OBB path is also at
			* top level. */
			setup_derived_state(sb->s_root->d_inode,
					    PERM_LEGACY_PRE_ROOT, 0,
					    AID_ROOT, AID_SDCARD_R,
					    00771);
			/* initialize the obbpath string and lookup
			* the path sb_info->obb_path will be deactivated
			* by path_put on sdcardfs_put_super */
			sb_info->obbpath_s = kzalloc(PATH_MAX,
						     GFP_KERNEL);
			snprintf(sb_info->obbpath_s, PATH_MAX,
				 "%s/obb", dev_name);
			err =  prepare_dir(sb_info->obbpath_s,
					   sb_info->options.fs_low_uid,
					   sb_info->options.fs_low_gid,
					   00755);
			if (err)
				pr_err("sdcardfs: %s: %d,\
				       error on creating %s\n",
			...
			break;
		case DERIVE_UNIFIED:
			/* Unified multiuser layout which places
			* secondary user_id under /Android/user
			* and shared OBB path under /Android/obb. */
			setup_derived_state(sb->s_root->d_inode,
					    PERM_ROOT, 0, AID_ROOT,
					    AID_SDCARD_R, 00771);
...
*mount_nodev_with_options(struct file_system_type *fs_type,
