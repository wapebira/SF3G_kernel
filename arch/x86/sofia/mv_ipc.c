...
irqreturn_t mbox_connect_handler(int irq, void *data)
{
	...
	if(db_entry->ops->on_connect != NULL)
...
irqreturn_t mbox_event_handler(int irq, void *data)
{
	...
	if(db_entry->ops->on_event != NULL)
	db_entry->ops->on_event(info->token,
				event_info->event_id, db_entry->cookie);

	if( unlikely(atomic_read(&system_freezing_cnt) == 1 && suspend_step >= 7) ) {
		desc = irq_to_desc(irq);
		if(desc ) {
			//HIRQ 156 means "mb-mvpipe-at2-1", if linux recieved hirq-156, that means modem want to wakeup ril to 
			//deal with network communication such as an incoming call or a sms message. In this situation, we have to 
			//aquire a wakelock to prevent linux entering deepsleep, otherise hirq will be lost or delayed for sometime to send to ril.
			if( !strcmp(desc->action->name , "mb-mvpipe-at2-1") )
				wake_lock_timeout( &ipc_wake_lock,msecs_to_jiffies(2 * 1000) );
		}
	}
	return IRQ_HANDLED;
}
