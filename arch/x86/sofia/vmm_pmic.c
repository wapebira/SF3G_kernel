...
static struct rt_mutex pmic_access_mutex;
static DECLARE_WAIT_QUEUE_HEAD(pmic_wait_queue);
static int32_t vmm_pmic_reg_access_initialised;
static DECLARE_COMPLETION(vmm_pmic_reg_access_completion);
static int32_t vmm_pmic_reg_access_irq_done;

struct pmic_access_shared_data *vmm_pmic_get_shared_data(void)
{
...
EXPORT_SYMBOL(vmm_pmic_get_shared_data);
				...
				uint8_t *data, uint32_t size_in_byte)
{
	...
	rt_mutex_lock(&pmic_access_mutex);
	...
	pmic_access_shared_data->status = -1;
	vmm_pmic_reg_access_irq_done = 0;
	reinit_completion(&vmm_pmic_reg_access_completion);
	...
	preempt_enable();
	wait_for_completion(&vmm_pmic_reg_access_completion);
...
pmic_write_end:
	rt_mutex_unlock(&pmic_access_mutex);
	return result;
}
...
{
	vmm_pmic_reg_access_irq_done = 1;
	complete(&vmm_pmic_reg_access_completion);
	return IRQ_HANDLED;
}
u8 regaddr;
u8 val;
...
static int32_t vmm_pmic_probe(struct platform_device *pdev)
{
	...
	rt_mutex_init(&pmic_access_mutex);
