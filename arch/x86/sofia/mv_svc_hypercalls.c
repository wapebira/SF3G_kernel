...
int32_t mv_svc_enter_atomic_context(void)
{
	return mv_platform_service(VMM_ENTER_ATOMIC_CONTEXT,
		0, 0, 0,
		0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
EXPORT_SYMBOL(mv_svc_enter_atomic_context);
#endif

int32_t mv_svc_exit_atomic_context(void)
{
	return mv_platform_service(VMM_EXIT_ATOMIC_CONTEXT,
		0, 0, 0,
		0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
EXPORT_SYMBOL(mv_svc_exit_atomic_context);
#endif
