...
#define LOG_BUF_SHIFT		\
((CONFIG_LOG_BUF_SHIFT > 19) ? 19 : CONFIG_LOG_BUF_SHIFT)
...
char *sofia_last_log_get(unsigned *size)
{
	*size = LOG_BUF_LEN;
	return last_log_buf;
}
...
static void * __init last_log_vmap(phys_addr_t start, unsigned int page_count)
{
	...
	return vmap(pages, page_count + 1, VM_MAP,
		    pgprot_noncached(PAGE_KERNEL));
}

static int __init sofia_last_log_init(void)
{
	...
	early_log_size = log_pos > sizeof(early_log_buf) ?
			 sizeof(early_log_buf) : log_pos;
	...
	pr_info("0x%08x map to 0x%p and copy to 0x%p, size 0x%x early 0x%x (version 3.0)\n",
		virt_to_phys(buf), log_buf, last_log_buf,
		LOG_BUF_LEN, early_log_size);
...
early_initcall(sofia_last_log_init);

void sofia_last_log_text(char *text, size_t size)
{
