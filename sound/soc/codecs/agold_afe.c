...
static int agold_afe_get_ext_amp_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	afe_debug("%s :\n", __func__);
...
/* Read current ext amp status*/
static int agold_afe_get_ext_amp_val(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct agold_afe_data *agold_afe = snd_soc_codec_get_drvdata(codec);
	...
	mutex_lock(&codec->mutex);
	uinfo->value.integer.value[0] = agold_afe->spk_con;
	mutex_unlock(&codec->mutex);

	afe_debug("%s: Current spk_con = %ld\n", __func__,
			uinfo->value.integer.value[0]);
...
/* Set ext amp */
static int agold_afe_set_ext_amp_val(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct agold_afe_data *agold_afe = snd_soc_codec_get_drvdata(codec);
	...
	if (agold_afe->spk_pin < 0)
		return 0;

	mutex_lock(&codec->mutex);
	agold_afe->spk_con = uinfo->value.integer.value[0];
	mutex_unlock(&codec->mutex);

	if (agold_afe->spk_con < 0)
		return 0;

	afe_debug("%s : SPK CON set = %d\n", __func__,
			agold_afe->spk_con);

	if (1 == agold_afe->spk_con)
		gpiod_set_value(gpio_to_desc(agold_afe->spk_pin), 1);
	else
		gpiod_set_value(gpio_to_desc(agold_afe->spk_pin), 0);
...
static const struct snd_kcontrol_new agold_afe_snd_controls[] = {
	...
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "External Amplifier Control",
		.info = agold_afe_get_ext_amp_info,
		.get = agold_afe_get_ext_amp_val,
		.put = agold_afe_set_ext_amp_val
	},
				...
				const struct idi_device_id *id)
{
	...
	/* spk gpio */
	agold_afe->spk_pin = of_get_named_gpio_flags(np,
			PROP_SPK_AMP_NAME, 0, NULL);

	if (agold_afe->spk_pin <= 0) {
		afe_debug("%s: unable to get speaker node %s\n",
			  __func__, PROP_SPK_AMP_NAME);
	} else {
		afe_debug("%s: Get speaker node %s value: %d !!!\n",
			  __func__, PROP_SPK_AMP_NAME, agold_afe->spk_pin);
		ret = gpio_request(agold_afe->spk_pin, PROP_SPK_AMP_NAME);
		if (!ret) {
			afe_debug("req gpio_request success!:%d\n", ret);
			gpiod_direction_output(
				gpio_to_desc(agold_afe->spk_pin), 0);
		} else {
