		...
		enum dsp_audio_controls cmd, void *arg)
{
	...
	__pm_stay_awake(dsp_dev->ws);
	xgold_debug("DSP acquire wake lock\n");
...
RELEASE_WAKE_LOCK:
	__pm_relax(dsp_dev->ws);
	xgold_debug("DSP release wake lock\n");
...
static int dsp_audio_drv_probe(struct platform_device *pdev)
{
	...
	if (dsp_dev->interrupts[HW_PROBE]) {
	...
	dsp_dev->ws = wakeup_source_register("Audio-DSP");
...
static int dsp_audio_drv_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(dsp->ws);
	return 0;
}
