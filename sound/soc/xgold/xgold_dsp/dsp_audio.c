...
enum dsp_err_code dsp_audio_init(struct list_head *list)
{
	...
	xgold_debug("%s, Num of dsp's = %d", __func__, num_dsp);
	...
	U16 *p_command)
{
	...
	xgold_debug("in %s, command_id=%d, command_len=%d, dsp.num_dsp=%d\n", __func__, command_id, command_len, dsp.num_dsp);
	if (dsp.num_dsp == 2) {
		...
		if (0x7fff != is_command_id_supported) {
			...
			if (command_id == DSP_SBA_VB_HW_I2S2) {
				xgold_debug("in %s, Two I2S. Enable I2S2 power&clock domain!\n", __func__);
				if (*p_command != 0)
			...
			/* Enable I2S1 power&clock domain */
				...
				xgold_debug("in %s, Two I2S. Enable I2S1 power&clock domain!\n", __func__);
				if (*p_command != 0) {
					dsp.dsp_sba->p_dsp_common_data->
					i2s_set_power_state(
					dsp.dsp_sba->p_dsp_common_data->
		...
		/* Enable I2S2 power&clock domain */
			if (command_id == DSP_SBA_VB_HW_I2S2) {
				xgold_debug("in %s, One I2S. Enable I2S2 power&clock domain!\n", __func__);
				if (*p_command != 0)
					dsp.dsp_fba->p_dsp_common_data->
					i2s_set_power_state(
					dsp.dsp_fba->p_dsp_common_data->
					p_i2s_dev[XGOLD_I2S2],
					1);
			...
			/* Enable I2S1 power&clock domain */
				...
				if (*p_command != 0) {
					xgold_debug("in %s, One I2S. Enable I2S1 power&clock domain!, *p_command != 0\n", __func__);
					dsp.dsp_fba->p_dsp_common_data->
					i2s_set_power_state(
					dsp.dsp_fba->p_dsp_common_data->
					p_i2s_dev[XGOLD_I2S2],
